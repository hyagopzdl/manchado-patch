      ...
      `,
        ...
        function PlayerOverrideHistoryAdmin({ catalog }) {
          ...
          function dataSummary(row){let before=row.previousData||{},after=row.newData||{},parts=[];[["overall","Overall"],["market_value","Valor"],["attack","Ataque"],["defense","Defesa"]].forEach(([key,label])=>{if(String(before[key]??"")!==String(after[key]??""))parts.push(`${label}: ${before[key]??"—"} → ${after[key]??"—"}`)});let oldAttrs=before.attributes||{},newAttrs=after.attributes||{};Object.keys({...oldAttrs,...newAttrs}).forEach(key=>{if(String(oldAttrs[key]??"")!==String(newAttrs[key]??""))parts.push(`${key}: ${oldAttrs[key]??"—"} → ${newAttrs[key]??"—"}`)});return parts.slice(0,8)}
          ...
          return React.createElement("div",{style:{marginTop:20,paddingTop:18,borderTop:"1px solid var(--border)"}},
            ...
            React.createElement("div",{style:{display:"grid",gap:10,marginTop:12,maxHeight:560,overflow:"auto"}},rows.map(row=>{
              ...
              if(review){
                ...
                let approved=voteList("Aprovada por",approvals);if(approved)reviewChildren.push(approved);
                let rejected=voteList("Recusada por",rejections);if(rejected)reviewChildren.push(rejected);
                if(review.resolvedAt)reviewChildren.push(React.createElement("div",{key:"resolved",style:{fontSize:10.5,color:"var(--muted)"}},"Concluída em ",formatDate(review.resolvedAt)));
              }
              return React.createElement("div",{key:row.id,style:{padding:13,borderRadius:14,border:"1px solid var(--border)",background:"var(--surface-soft)"}},
                ...
                changes.length?React.createElement("div",{style:{display:"grid",gap:4,marginTop:9,fontSize:11.5}},changes.map((item,index)=>React.createElement("div",{key:index},item))):null,
