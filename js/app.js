        ...
        function parseSingleRosterTxt(text, catalog, ownership) {
          ...
          entries.forEach((entry)=>{
            ...
            if(seen.has(entry.playerId)){
              entry.status="duplicate";
              let previous=entries.find((candidate)=>candidate!==entry&&candidate.playerId===entry.playerId&&candidate.status!=="owned");
              if(previous)previous.status="duplicate";
            }else seen.set(entry.playerId,true);
          });
          let matchedEntries=entries.filter((entry)=>entry.playerId&&entry.status==="matched");
          let ignoredOwnedEntries=entries.filter((entry)=>entry.status==="owned");
          let matchedPlayers=matchedEntries.map((entry)=>(catalog||[]).find((player)=>String(player.id)===String(entry.playerId))).filter(Boolean);
          ...
          return {entries,total:entries.length,matchedEntries,ignoredOwnedEntries,ignoredOwnedCount:ignoredOwnedEntries.length,usableCount:matchedEntries.length,matchedPlayers,metrics:balancedRosterMetrics(matchedPlayers),groupCounts};
        }
              ...
              lateJoinOpen&&React.createElement("div",{style:{marginTop:16,paddingTop:16,borderTop:"1px solid var(--border)",display:"grid",gap:12}},
                ...
                lateJoinMode==="txt"&&React.createElement(React.Fragment,null,
                  ...
                  lateJoinTxtPreview&&React.createElement("div",{style:{display:"grid",gap:10}},
                    ...
                    (lateJoinTxtPreview.ignoredOwnedCount||0)>0&&React.createElement("div",{style:{padding:10,borderRadius:10,background:"color-mix(in srgb,#ffbb26 10%,var(--surface))",border:"1px solid color-mix(in srgb,#ffbb26 32%,var(--border))",fontSize:11.5,lineHeight:1.45}},`${lateJoinTxtPreview.ignoredOwnedCount} jogador${lateJoinTxtPreview.ignoredOwnedCount===1?" do TXT já pertence":"es do TXT já pertencem"} a outro time e ${lateJoinTxtPreview.ignoredOwnedCount===1?"será ignorado":"serão ignorados"}. Você pode seguir com os ${lateJoinTxtPreview.usableCount||0} disponíveis, mesmo com o elenco abaixo de 23.`),
