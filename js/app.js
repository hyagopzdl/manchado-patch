...
O elenco ficará abaixo de 23 jogadores e poderá ser completado depois.`;if(!window.confirm(message))return;setLateJoinBusy(true);try{await onImportLateJoinTxt({profileId:lateJoinProfileId,budget:lateJoinBudget,entries:usableEntries.map((entry)=>({playerId:entry.playerId,squadRole:entry.squadRole}))});setLateJoinOpen(false);setLateJoinTxt("");setLateJoinTxtPreview(null);window.alert(`Participante adicionado com ${usableEntries.length} jogador${usableEntries.length===1?"":"es"}.${replacements.length?` ${replacements.length} substituição${replacements.length===1?" foi aplicada":"ões foram aplicadas"}.`:""}${ignoredOwned.length?` ${ignoredOwned.length} indisponível${ignoredOwned.length===1?" foi ignorado":"is foram ignorados"}.`:""}`);}catch(error){console.error("late join txt import failed",error);window.alert(error&&error.message?error.message:"Não foi possível adicionar o participante pelo TXT.");}finally{setLateJoinBusy(false);}}
        ...
        function HeadToHeadStats({ tournament, currentTeam, opponentTeam, profiles, teams, tournaments = [], onOpenChampionshipSummary }) {
          ...
          function trophySide(label, profile, trophies, align) { return React.createElement("div",{style:{minWidth:0,textAlign:align}},React.createElement("div",{style:{fontSize:11,color:"var(--muted)",marginBottom:8}},label),trophies.length?React.createElement("div",{style:{display:"flex",gap:7,justifyContent:align==="right"?"flex-end":"flex-start",flexWrap:"wrap"}},trophies.slice(0,6).map(({tournament:item})=>React.createElement("button",{key:item.id,title:item.name,onClick:()=>onOpenChampionshipSummary&&onOpenChampionshipSummary(item),style:{width:48,height:48,borderRadius:12,border:"1px solid var(--border)",background:"var(--surface-soft)",display:"grid",placeItems:"center",padding:5,cursor:onOpenChampionshipSummary?"pointer":"default"}},React.createElement(TrophyAsset,{tournament:item,size:34})))):React.createElement("div",{style:{fontSize:12,color:"var(--muted)",lineHeight:1.4}},"Nenhum troféu conquistado")); }
          ...
          let winnerSide=mine.wins===other.wins?null:(mine.wins>other.wins?"left":"right");
          function resultSegment(value,pct,label,kind,highlight){let bg=kind==="win"?"color-mix(in srgb,var(--green) 18%,var(--surface-soft))":kind==="loss"?"color-mix(in srgb,var(--danger) 14%,var(--surface-soft))":"var(--surface-soft)";let color=kind==="win"?"var(--green)":kind==="loss"?"var(--danger)":"var(--heading)";return React.createElement("div",{style:{flex:`${Math.max(pct,18)} 1 0`,minWidth:0,padding:"14px 10px",background:bg,textAlign:"center",borderRadius:12,boxShadow:highlight?"inset 0 0 0 1px color-mix(in srgb,var(--green) 50%,transparent)":"none"}},React.createElement("div",{style:{display:"inline-grid",placeItems:"center",minWidth:34,height:34,padding:"0 8px",borderRadius:10,background:highlight?"var(--heading)":"transparent",color:highlight?"var(--bg)":color,fontSize:20,fontWeight:900,fontVariantNumeric:"tabular-nums",lineHeight:1}},value),React.createElement("div",{style:{fontSize:10.5,fontWeight:800,marginTop:7,color}},label),React.createElement("div",{style:{fontSize:10.5,color:"var(--muted)",marginTop:3}},`${Math.round(pct)}%`));}
          return React.createElement("div",null,
            React.createElement("section",{className:"family-card",style:{padding:"clamp(18px,4vw,26px)",marginBottom:16}},
              React.createElement("div",{style:{display:"flex",alignItems:"center",justifyContent:"space-between",gap:14,marginBottom:16}},
                React.createElement("div",{style:{display:"flex",alignItems:"center",gap:9,minWidth:0}},avatarForTeam(currentTeam,44),React.createElement("strong",{style:{fontSize:14,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}},currentProfile.name||currentTeam.name)),
                React.createElement("div",{style:{fontSize:11,color:"var(--muted)",fontWeight:750,whiteSpace:"nowrap"}},`${totalMatches} ${totalMatches===1?"confronto":"confrontos"}`),
                React.createElement("div",{style:{display:"flex",alignItems:"center",gap:9,minWidth:0,justifyContent:"flex-end"}},React.createElement("strong",{style:{fontSize:14,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",textAlign:"right"}},opponentProfile.name||opponentTeam.name),avatarForTeam(opponentTeam,44))
              ),
              React.createElement("div",{style:{display:"flex",gap:5,alignItems:"stretch"}},resultSegment(mine.wins,winPct,"Vitórias","win",winnerSide==="left"),resultSegment(mine.draws,drawPct,"Empates","draw",false),resultSegment(other.wins,lossPct,"Derrotas","loss",winnerSide==="right")),
              React.createElement("div",{style:{display:"flex",justifyContent:"center",alignItems:"center",gap:8,marginTop:15,fontSize:12,color:"var(--muted)"}},React.createElement("span",null,"Gols"),React.createElement("strong",{style:{fontSize:16,color:"var(--heading)",fontVariantNumeric:"tabular-nums"}},`${mine.gf} × ${other.gf}`))
            ),
            React.createElement("section",{className:"family-card",style:{padding:"18px 20px",marginBottom:16}},React.createElement("div",{style:{fontSize:12,fontWeight:800,marginBottom:12}},"Seus últimos confrontos"),React.createElement("div",{style:{display:"flex",gap:8,flexWrap:"wrap"}},lastFive.map((result,index)=>{let latest=index===lastFive.length-1;return React.createElement("span",{key:index,title:`${result==="V"?"Vitória":result==="D"?"Derrota":"Empate"}${latest?" · Mais recente":""}`,style:{width:32,height:32,borderRadius:9,display:"grid",placeItems:"center",fontSize:12,fontWeight:900,background:result==="V"?"color-mix(in srgb,var(--green) 16%,var(--surface-soft))":result==="D"?"color-mix(in srgb,var(--danger) 15%,var(--surface-soft))":"var(--surface-soft)",color:result==="V"?"var(--green)":result==="D"?"var(--danger)":"var(--muted)",border:latest?"2px solid var(--heading)":"1px solid var(--border)",boxShadow:latest?"0 0 0 2px color-mix(in srgb,var(--heading) 8%,transparent)":"none"}},result)}))),
            ...
            matches.length>1 && React.createElement("section",{style:{marginBottom:20}},React.createElement("div",{style:{fontSize:16,fontWeight:850,margin:"0 0 10px"}},"Destaques"),React.createElement("div",{style:{display:"grid",gridTemplateColumns:"repeat(2,minmax(0,1fr))",gap:10}},
              React.createElement("div",{className:"family-card",style:{padding:15}},React.createElement("div",{style:{fontSize:11,color:"var(--muted)",marginBottom:6}},"Maior vitória"),React.createElement("strong",{style:{fontSize:17}},biggestResult(currentTeam,"win"))),
              React.createElement("div",{className:"family-card",style:{padding:15}},React.createElement("div",{style:{fontSize:11,color:"var(--muted)",marginBottom:6}},"Maior derrota"),React.createElement("strong",{style:{fontSize:17}},biggestResult(currentTeam,"loss"))),
