      (() => {
        ...
        function to() {
          ...
          }), [baseCatalog, playerCatalogOverrides, overridesEnabled]);
          let xe = X(() => {
            let o = new Map();
            ...
            try {
              ...
              if (!targetTournamentId && tournamentName) {
                let tournament = {
                  ...
                  teamIds: [],
                  createdAt: Date.now(),
                  marketSettings: { depreciationPct: 10, initialRosterDepreciationPct: 50, isOpen: true, freePlayerOverallLimit: { enabled:false, minOverall:1, maxOverall:99 }, playerTradeLock:{ enabled:false, gamesRequired:50 }, minimumGamesForMarket:{ enabled:false, gamesRequired:0 }, releaseClause:{ enabled:false, multiplier:2, revalueMultiplier:1.5, protectedGames:50, maxSufferedPerTeam:2, blockDirectRepurchase:true, shieldingEnabled:true, shieldingConversion:1, shieldingMaxPct:100 }, playerReportsEnabled:true, playerOverridesEnabled:true },
          ...
          function playerTradeLockStatus(playerId, teamId, tournament = R, ownershipValue = c, transfersValue = k, matchesValue = null) {
            let sourceMatches = matchesValue || (tournament && tournament.context && Array.isArray(tournament.context.matches) ? tournament.context.matches : (Array.isArray(tournament && tournament.matches) ? tournament.matches : []));
            let clauseLock = releaseClauseContractStatus(playerId, teamId, tournament, sourceMatches);
            if (clauseLock.locked) return clauseLock;
            return window.ManchaApp.MarketFeature.playerTradeLockStatus(playerId, teamId, tournament, ownershipValue, transfersValue, sourceMatches);
          }
          ...
          function marketMinimumGamesSettings(tournament = R) {
            let settings = tournament && tournament.marketSettings && tournament.marketSettings.minimumGamesForMarket && typeof tournament.marketSettings.minimumGamesForMarket === "object" ? tournament.marketSettings.minimumGamesForMarket : {};
            return { enabled: settings.enabled === true, gamesRequired: Math.max(0, Math.round(Number(settings.gamesRequired) || 0)) };
          }
          function teamPlayedGames(teamId, tournament = R) {
            if (!teamId || !tournament) return 0;
            let matches = tournament.context && Array.isArray(tournament.context.matches) ? tournament.context.matches : (Array.isArray(tournament.matches) ? tournament.matches : []);
            return matches.filter((match) => match && match.played === true && !match.bye && (String(match.homeId) === String(teamId) || String(match.awayId) === String(teamId))).length;
          }
          function marketMinimumGamesStatus(teamId, tournament = R) {
            let settings = marketMinimumGamesSettings(tournament);
            let played = teamPlayedGames(teamId, tournament);
            let remaining = Math.max(0, settings.gamesRequired - played);
            return { enabled:settings.enabled, gamesRequired:settings.gamesRequired, played, remaining, blocked:settings.enabled && remaining > 0 };
          }
          function marketMinimumGamesMessage(status) {
            if (!status || !status.blocked) return "";
            return `Você precisa disputar pelo menos ${status.gamesRequired} jogos neste campeonato antes de comprar ou vender no mercado. Faltam ${status.remaining} jogo${status.remaining === 1 ? "" : "s"}.`;
          }
          ...
          function releaseClauseContractStatus(playerId, teamId, tournament = R, matchesValue = null) {
            let state=releaseClauseState(tournament), contracts=state.contracts&&typeof state.contracts==="object"?state.contracts:{}, contract=contracts[String(playerId)]||null;
            if(!contract||String(contract.teamId||"")!==String(teamId||"")) return {locked:false,gamesRemaining:0,reason:null};
            let played=teamPlayedGames(teamId,tournament), start=Math.max(0,Number(contract.startGames)||0), required=Math.max(0,Number(contract.gamesRequired)||0), completed=Math.max(0,played-start), remaining=Math.max(0,required-completed);
            return {locked:remaining>0,gamesRemaining:remaining,gamesCompleted:completed,gamesRequired:required,reason:"release_clause_contract",contract};
          }
          function releaseClauseInfo(player, tournament = R) {
            let settings=releaseClauseSettings(tournament), owner=player&&c?c[String(player.id)]:null, ownerTeamId=owner&&owner.teamId, ownerTeam=ownerTeamId?p.find(team=>team&&String(team.id)===String(ownerTeamId)):null;
            let state=releaseClauseState(tournament), shieldingMap=state.shielding&&typeof state.shielding==="object"?state.shielding:{}, shieldEntry=shieldingMap[String(player&&player.id)]||null;
            let base=Math.ceil((Number(player&&player.value)||0)*settings.multiplier), shielding=shieldEntry&&String(shieldEntry.teamId||"")===String(ownerTeamId||"")?Math.max(0,Number(shieldEntry.amount)||0):0;
            let total=Math.ceil(base+shielding), newValue=Math.ceil((Number(player&&player.value)||0)*settings.revalueMultiplier), sufferedMap=state.sufferedByTeam&&typeof state.sufferedByTeam==="object"?state.sufferedByTeam:{}, suffered=Math.max(0,Number(sufferedMap[String(ownerTeamId)]||0));
            let contract=ownerTeamId?releaseClauseContractStatus(player&&player.id,ownerTeamId,tournament):{locked:false,gamesRemaining:0};
            let own=!!(ProfileTeam&&ownerTeamId&&String(ProfileTeam.id)===String(ownerTeamId)), other=!!(ProfileTeam&&ownerTeamId&&String(ProfileTeam.id)!==String(ownerTeamId));
            let buyerAccess=ProfileTeam?marketMinimumGamesStatus(ProfileTeam.id,tournament):{blocked:true}, sellerAccess=ownerTeamId?marketMinimumGamesStatus(ownerTeamId,tournament):{blocked:true};
            let sellerSize=ownerTeamId?Object.values(c||{}).filter(item=>item&&String(item.teamId)===String(ownerTeamId)).length:0, buyerSize=ProfileTeam?Object.values(c||{}).filter(item=>item&&String(item.teamId)===String(ProfileTeam.id)).length:0;
            let minPlayers=Math.max(0,Number(tournament&&tournament.rosterSettings&&tournament.rosterSettings.minPlayers!=null?tournament.rosterSettings.minPlayers:23)||0), maxPlayers=Math.max(1,Number(tournament&&tournament.rosterSettings&&tournament.rosterSettings.maxPlayers!=null?tournament.rosterSettings.maxPlayers:30)||30);
            let blockedReason=null;
            if(!settings.enabled) blockedReason="Mecânica desabilitada"; else if(!(tournament&&tournament.marketSettings&&tournament.marketSettings.isOpen!==false)) blockedReason="Mercado fechado"; else if(!other) blockedReason="Disponível apenas para jogadores de outro time"; else if(contract.locked) blockedReason=`Contrato protegido · ${contract.gamesRemaining} jogos restantes`; else if(settings.maxSufferedPerTeam>0&&suffered>=settings.maxSufferedPerTeam) blockedReason="Time protegido contra novas multas"; else if(buyerAccess.blocked) blockedReason=marketMinimumGamesMessage(buyerAccess); else if(sellerAccess.blocked) blockedReason="O vendedor ainda não atingiu o mínimo de jogos para negociar."; else if(sellerSize<=minPlayers) blockedReason=`O time precisa manter pelo menos ${minPlayers} jogadores.`; else if(buyerSize>=maxPlayers) blockedReason=`Seu elenco atingiu o limite de ${maxPlayers} jogadores.`; else if(ProfileTeam&&Number(ProfileTeam.budget||0)<total) blockedReason="Saldo insuficiente";
            return {settings,owner,ownerTeamId,ownerTeam,own,other,base,shielding,total,newValue,suffered,contract,blocked:!!blockedReason,blockedReason};
          }
          async function payReleaseClause(player) {
            if(!player||!R||!ProfileTeam||!te)return;
            let info=releaseClauseInfo(player,R); if(info.blocked){window.alert(info.blockedReason||"Não é possível pagar a multa deste jogador.");return;}
            let sellerName=info.ownerTeam&&info.ownerTeam.name||"outro time";
            if(!window.confirm(`Pagar a multa rescisória de ${player.name}?\n\nValor atual: ${L(player.value||0)}\nMulta: ${L(info.total)}\nNovo valor: ${L(info.newValue)}\nContrato protegido: ${info.settings.protectedGames} jogos\n\n${sellerName} receberá ${L(info.total)} imediatamente.`))return;
            try{let fn=window.ManchaApp&&window.ManchaApp.payReleaseClause;if(typeof fn!=="function")throw new Error("Execute o SQL da Multa rescisória e atualize js/supabase.js.");await fn({tournamentId:R.id,playerId:player.id,playerName:player.name,buyerTeamId:ProfileTeam.id,sellerTeamId:info.ownerTeamId,marketValue:Number(player.value)||0,clauseAmount:info.total,actorProfileId:te.id});be(null);window.alert(`${player.name} foi contratado por ${L(info.total)} pagando a multa rescisória.`);}catch(error){console.error("release clause failed",error);window.alert(`Não foi possível pagar a multa. ${error&&error.message?error.message:"Tente novamente."}`);}
          }
          async function increaseReleaseClauseShielding(player) {
            if(!player||!R||!ProfileTeam||!te)return; let info=releaseClauseInfo(player,R), settings=info.settings;
            if(!settings.enabled||!settings.shieldingEnabled){window.alert("A blindagem da multa está desabilitada.");return;} if(!info.own){window.alert("Você só pode aumentar a multa de jogadores do seu próprio elenco.");return;}
            let maxShield=info.base*(settings.shieldingMaxPct/100), remaining=Math.max(0,maxShield-info.shielding); if(remaining<=0){window.alert("Este jogador já atingiu a blindagem máxima configurada.");return;}
            let raw=window.prompt(`Quanto deseja investir na multa de ${player.name}?\n\nMulta atual: ${L(info.total)}\nBlindagem atual: +${L(info.shielding)}\nSaldo: ${L(ProfileTeam.budget||0)}\n\nCada 1M gasto adiciona ${settings.shieldingConversion}M à multa.`); if(raw==null)return; let spend=Number(String(raw).replace(",",".")); if(!Number.isFinite(spend)||spend<=0){window.alert("Informe um valor válido.");return;}
            if(spend>Number(ProfileTeam.budget||0)){window.alert("Saldo insuficiente para essa blindagem.");return;} if(spend*settings.shieldingConversion>remaining+.0001){window.alert(`A blindagem máxima restante é ${L(remaining)}. Reduza o investimento.`);return;}
            try{let fn=window.ManchaApp&&window.ManchaApp.increaseReleaseClauseShielding;if(typeof fn!=="function")throw new Error("Execute o SQL da Multa rescisória e atualize js/supabase.js.");await fn({tournamentId:R.id,playerId:player.id,teamId:ProfileTeam.id,marketValue:Number(player.value)||0,spendAmount:spend,expectedCurrentShield:info.shielding,actorProfileId:te.id});be(null);window.alert(`Multa de ${player.name} aumentada com sucesso.`);}catch(error){console.error("release clause shielding failed",error);window.alert(`Não foi possível aumentar a multa. ${error&&error.message?error.message:"Tente novamente."}`);}
          }
          ...
          function kt(o) {
            ...
            if (accessStatus.blocked) { window.alert(marketMinimumGamesMessage(accessStatus)); return; }
          ...
          async function sendTradeOffer(player, amount) {
            ...
            let buyerAccess = marketMinimumGamesStatus(ProfileTeam.id, marketTournament || R);
            if (buyerAccess.blocked) { window.alert(marketMinimumGamesMessage(buyerAccess)); return; }
            let status = Pe(player), sellerTeamId = status.teamId;
            let sellerMarketAccess = sellerTeamId ? marketMinimumGamesStatus(sellerTeamId, marketTournament || R) : null;
            if (sellerMarketAccess && sellerMarketAccess.blocked) { window.alert("O vendedor ainda não atingiu o número mínimo de jogos para negociar no mercado."); return; }
            ...
            let result = await mutateTradeOffersAtomic(`offer-send:${marketTournamentId}:${player.id}:${ProfileTeam.id}`, (offers, tournament, fail) => {
              ...
              let buyerAccessAtomic = marketMinimumGamesStatus(ProfileTeam.id, tournament);
              let sellerAccessAtomic = marketMinimumGamesStatus(actualSellerId, tournament);
              if (buyerAccessAtomic.blocked || sellerAccessAtomic.blocked) { fail("minimum_games_market"); return null; }
            ...
            let messages = {
              ...
              minimum_games_market:"Você ainda não atingiu o número mínimo de jogos para negociar no mercado.",
          ...
          async function updateTradeOffer(offerId, action, amount) {
            ...
            if (action === "counter") { let accessStatus = marketMinimumGamesStatus(ProfileTeam.id, marketTournament || R); if (accessStatus.blocked) { window.alert(marketMinimumGamesMessage(accessStatus)); return; } }
            let result = await mutateTradeOffersAtomic(`offer-update:${marketTournamentId}:${offerId}`, (offers, tournament, fail) => {
              ...
              if (action === "counter") {
                let accessStatus = marketMinimumGamesStatus(ProfileTeam.id, tournament);
                if (accessStatus.blocked) { fail("minimum_games_market"); return null; }
              }
            ...
            if (!result.committed && result.reason !== "busy") {
              let messages = { unavailable:"A proposta não está mais disponível.", forbidden:"Você não pode alterar esta proposta.", not_your_turn:"Aguarde a resposta do outro usuário.", player_trade_locked:"Este jogador ainda não cumpriu o número mínimo de jogos no time atual.", minimum_games_market:"Você ainda não atingiu o número mínimo de jogos para negociar no mercado." };
          ...
          function acceptTradeOffer(offerId) {
            ...
            let localBuyerAccess = marketMinimumGamesStatus(localBuyer.id, R);
            let localSellerAccess = marketMinimumGamesStatus(localSeller.id, R);
            if (localBuyerAccess.blocked || localSellerAccess.blocked) {
              let blockedStatus = String(ProfileTeam.id) === String(localBuyer.id) ? localBuyerAccess : localSellerAccess;
              window.alert(marketMinimumGamesMessage(blockedStatus.blocked ? blockedStatus : (localBuyerAccess.blocked ? localBuyerAccess : localSellerAccess)));
              return;
            }
            ...
            db.ref("pes/tournaments").transaction((serverValue) => {
              ...
              if (marketMinimumGamesStatus(buyer.id, tournament).blocked || marketMinimumGamesStatus(seller.id, tournament).blocked) { failureReason = "minimum_games_market"; return; }
              ...
              let messages = {
                ...
                minimum_games_market: "Um dos participantes ainda não atingiu o número mínimo de jogos para negociar. A proposta continuará aberta."
              };
          ...
          function Ft(o, i, y) {
            ...
            let buyerAccess = marketMinimumGamesStatus(y, R);
            if (buyerAccess.blocked) { window.alert(marketMinimumGamesMessage(buyerAccess)); return; }
            ...
            let applyPurchase = (tournament) => {
              ...
              if (marketMinimumGamesStatus(buyer.id, tournament).blocked) { failureReason="minimum_games_market"; return null; }
              ...
              if (currentOwner && currentOwner.teamId && String(currentOwner.teamId) !== String(buyer.id) && marketMinimumGamesStatus(currentOwner.teamId, tournament).blocked) { failureReason="minimum_games_market"; return null; }
          ...
          function wt(o, i, y) {
            if (!marketAccessSettings(R).isOpen) { window.alert("O mercado está fechado pela administração."); return; }
            let accessStatus = marketMinimumGamesStatus(i, R);
            if (accessStatus.blocked) { window.alert(marketMinimumGamesMessage(accessStatus)); return; }
          ...
          function It(o) {
            ...
            if (!marketAccessSettings(R).isOpen) { window.alert("O mercado está fechado pela administração."); return; }
            let accessStatus = marketMinimumGamesStatus(ProfileTeam.id, R);
            if (accessStatus.blocked) { window.alert(marketMinimumGamesMessage(accessStatus)); return; }
          ...
          function confirmMarketSale(o) {
            ...
            let applySale = (tournament) => {
              ...
              if (marketMinimumGamesStatus(teamId, tournament).blocked) { failureReason = "minimum_games_market"; return null; }
            ...
            db.ref("pes/tournaments").transaction((serverList) => {
              ...
              else if (!committed) window.alert(failureReason === "minimum_games_market" ? "Você ainda não atingiu o número mínimo de jogos para negociar no mercado." : failureReason === "player_trade_locked" ? "Este jogador ainda não cumpriu o número mínimo de jogos no seu time." : failureReason === "base_roster_minimum" ? "A venda foi bloqueada porque o time precisa manter o mínimo de jogadores do elenco-base." : "A venda foi cancelada porque o jogador não pertence mais ao seu elenco.");
              else {
                applyConfirmedTournamentSnapshot(snapshot);
                setSaleModal(null);
          ...
          async function importLateJoinParticipantTxt(options) {
            ...
            let ids=entries.map((entry)=>String(entry.playerId||"")).filter(Boolean);
            if(!ids.length)throw new Error("Nenhum jogador disponível foi selecionado para este elenco.");
            if(new Set(ids).size!==ids.length)throw new Error("Existem jogadores duplicados ou não resolvidos no TXT.");
            let ownership=R.context&&R.context.ownership&&typeof R.context.ownership==="object"?R.context.ownership:c;
            if(ids.some((id)=>ownership&&ownership[id]&&ownership[id].teamId))throw new Error("Um jogador ficou indisponível desde a prévia. Processe o TXT novamente antes de confirmar.");
            let players=ids.map((id)=>n.find((player)=>player&&String(player.id)===id)).filter(Boolean);if(players.length!==ids.length)throw new Error("Nem todos os jogadores importados existem no catálogo atual.");
          ...
          function updateMarketAccessRules(isOpen, limitEnabled, minOverall, maxOverall, tradeLockEnabled, tradeLockGames, playerReportsEnabled, playerOverridesEnabled, minimumGamesEnabled, minimumGamesRequired, releaseClauseConfig) {
            if (!R) return;
            ...
            let rc=releaseClauseConfig&&typeof releaseClauseConfig==="object"?releaseClauseConfig:{}; let nextSettings = { ...(R.marketSettings || {}), isOpen: isOpen === true, playerReportsEnabled: playerReportsEnabled !== false, playerOverridesEnabled: playerOverridesEnabled !== false, freePlayerOverallLimit: { enabled: limitEnabled === true, minOverall: normalizedMin, maxOverall: normalizedMax }, playerTradeLock:{ enabled:tradeLockEnabled === true, gamesRequired:Math.max(0,Math.round(Number(tradeLockGames)||0)) }, minimumGamesForMarket:{ enabled:minimumGamesEnabled === true, gamesRequired:Math.max(0,Math.round(Number(minimumGamesRequired)||0)) }, releaseClause:{ enabled:rc.enabled===true, multiplier:Math.max(1,Number(rc.multiplier)||2), revalueMultiplier:Math.max(1,Number(rc.revalueMultiplier)||1.5), protectedGames:Math.max(0,Math.round(Number(rc.protectedGames)||0)), maxSufferedPerTeam:Math.max(0,Math.round(Number(rc.maxSufferedPerTeam)||0)), blockDirectRepurchase:rc.blockDirectRepurchase!==false, shieldingEnabled:rc.shieldingEnabled!==false, shieldingConversion:Math.max(.01,Number(rc.shieldingConversion)||1), shieldingMaxPct:Math.max(0,Number(rc.shieldingMaxPct)||0) } };
      ...
      `,
                    ...
                    Me &&
                      React.createElement(ro, {
                        ...
                        activeTeam: ProfileTeam,
                        releaseClauseOf: (player) => releaseClauseInfo(player,R),
                        onPayReleaseClause: payReleaseClause,
                        onIncreaseReleaseClause: increaseReleaseClauseShielding,
        ...
        function playerDetailAttributes(player) {
          let attrs = player && player.attrs || {};
          return [
            ["Ataque", player && player.attack], ["Defesa", player && player.defense], ["Equilíbrio", attrs.balance], ["Fôlego", attrs.stamina],
            ["Velocidade máxima", attrs.topSpeed], ["Aceleração", attrs.acceleration], ["Resposta", attrs.response], ["Agilidade", attrs.agility],
            ["Precisão no drible", attrs.dribbleAccuracy], ["Velocidade no drible", attrs.dribbleSpeed],
            ["Precisão no passe curto", attrs.shortPassAccuracy], ["Velocidade do passe curto", attrs.shortPassSpeed],
            ["Precisão no passe longo", attrs.longPassAccuracy], ["Velocidade do passe longo", attrs.longPassSpeed],
            ["Precisão do chute", attrs.shotAccuracy], ["Força do chute", attrs.shotPower], ["Técnica de chute", attrs.shotTechnique],
            ["Precisão em cobranças de falta", attrs.freeKickAccuracy], ["Curva", attrs.curve], ["Cabeceio", attrs.heading], ["Impulsão", attrs.jump],
            ["Técnica", attrs.technique], ["Agressividade", attrs.aggression], ["Mentalidade", attrs.mentality],
            ["Habilidade de goleiro", attrs.goalKeepingSkills], ["Trabalho em equipe", attrs.teamwork],
            ["Consistência", attrs.consistency, 8], ["Condição física", attrs.conditionFitness, 8],
            ["Precisão com o pé ruim", attrs.weakFootAccuracy, 8], ["Frequência de uso do pé ruim", attrs.weakFootFrequency, 8],
          ];
        }
        function comparisonIndicator(leftValue, rightValue, side) {
          let leftNumber=Number(leftValue), rightNumber=Number(rightValue);
          if (!Number.isFinite(leftNumber) || !Number.isFinite(rightNumber) || leftNumber === rightNumber) return null;
          let sideIsHigher = side === "left" ? leftNumber > rightNumber : rightNumber > leftNumber;
          return { symbol:sideIsHigher ? "▲" : "▼", color:sideIsHigher ? "var(--green)" : "var(--danger)" };
        }
        function ComparisonTriangle({ leftValue, rightValue, side, size = 13 }) {
          let indicator=comparisonIndicator(leftValue,rightValue,side);
          if (!indicator) return null;
          return React.createElement("span", {
            "aria-hidden":"true",
            style:{ display:"inline-flex", alignItems:"center", justifyContent:"center", flex:"0 0 auto", color:indicator.color, fontSize:size, fontWeight:900, lineHeight:1, transform:"translateY(-.5px)" }
          }, indicator.symbol);
        }
        function ComparisonAttributeRow({ label, leftValue, rightValue, max = 99 }) {
          ...
          return React.createElement("div", { style:{ minHeight:44, display:"grid", gridTemplateColumns:"minmax(54px,.78fr) minmax(148px,1.8fr) minmax(54px,.78fr)", alignItems:"center", gap:8, padding:"9px 2px", borderBottom:"1px solid var(--border)" } },
            ...
            React.createElement("span", { style:{ minWidth:0, display:"grid", gridTemplateColumns:"18px minmax(0,auto) 18px", alignItems:"center", justifyContent:"center", columnGap:5, color:"var(--heading)", fontSize:13, lineHeight:1.25, textAlign:"center" } },
              React.createElement(ComparisonTriangle,{leftValue:leftNumber,rightValue:rightNumber,side:"left",size:13}),
              React.createElement("span",{style:{minWidth:0}},label),
              React.createElement(ComparisonTriangle,{leftValue:leftNumber,rightValue:rightNumber,side:"right",size:13})
            ),
        ...
        function ro({ player: e, catalog = [], statusOf, teamById, onClose: t, onOffer, onBuy, activeTeam, onReport, playerReportsEnabled = true, releaseClauseOf, onPayReleaseClause, onIncreaseReleaseClause }) {
          ...
          let attributes = playerDetailAttributes(e);
          let compareAttributes = comparePlayer ? playerDetailAttributes(comparePlayer) : [];
          let normalizeSearch = (value) => String(value || "").normalize("NFD").replace(/[\u0300-\u036f]/g,"").toLowerCase().trim();
          let normalizedQuery = normalizeSearch(compareQuery);
          let compareCandidates = X(() => {
            let source = (Array.isArray(catalog) ? catalog : []).filter((player) => player && String(player.id) !== String(e && e.id));
            if (normalizedQuery) {
              source = source.filter((player) => normalizeSearch(`${player.name || ""} ${player.club || ""} ${player.position || ""}`).includes(normalizedQuery));
              source.sort((left,right) => {
                let leftName=normalizeSearch(left.name), rightName=normalizeSearch(right.name);
                let leftStarts=leftName.startsWith(normalizedQuery)?0:1, rightStarts=rightName.startsWith(normalizedQuery)?0:1;
                return leftStarts-rightStarts || Number(right.overall||0)-Number(left.overall||0) || leftName.localeCompare(rightName,"pt-BR");
              });
            } else source.sort((left,right)=>Number(right.overall||0)-Number(left.overall||0) || String(left.name||"").localeCompare(String(right.name||""),"pt-BR"));
            return source.slice(0,normalizedQuery?30:16);
          },[catalog,e&&e.id,normalizedQuery]);
          let sourceLabel = (player) => {
            let status = statusOf && player ? statusOf(player) : null;
            if (!status || status.kind === "free") return "Mercado";
            let team = teamById && status.teamId ? teamById(status.teamId) : null;
            return team && team.name ? team.name : "Elenco";
          };
          ...
          let clauseAction = !comparePlayer && clauseInfo && clauseInfo.settings && clauseInfo.settings.enabled && (clauseInfo.other || clauseInfo.own) ? React.createElement("div",{style:{...E,padding:14,marginTop:10}},
            ...
            clauseInfo.own&&clauseInfo.settings.shieldingEnabled&&React.createElement("button",{className:"tapbtn",onClick:()=>onIncreaseReleaseClause&&onIncreaseReleaseClause(e),style:{...M,width:"100%",marginTop:12}},"Aumentar multa")
          ):null;
          let footerAction = !comparePlayer ? React.createElement("div",{style:{width:"100%"}},normalAction,clauseAction) : null;
          let compactPlayerHeader = (player, isRight, opponent) => {
            ...
            let source=sourceLabel(player);
            let teamLine=source && source !== "Mercado" ? `Time atual · ${source}` : null;
            let overallBlock = React.createElement("div", { style:{ display:"flex", flexDirection:isRight?"row-reverse":"row", alignItems:"flex-start", justifyContent:isRight?"flex-start":"flex-start", gap:7, width:"100%" } },
              React.createElement("strong", { style:{ fontSize:"clamp(38px,10vw,54px)", lineHeight:.92, letterSpacing:"-2.4px", color:overallColor(player.overall), fontVariantNumeric:"tabular-nums", fontWeight:900 } }, player.overall || "—"),
              React.createElement(ComparisonTriangle,{leftValue:isRight?opponentOverall:playerOverall,rightValue:isRight?playerOverall:opponentOverall,side,size:14})
            );
            let valueBlock = React.createElement("div", { style:{ display:"inline-flex", flexDirection:isRight?"row-reverse":"row", alignItems:"center", gap:6, marginTop:6, color:"var(--heading)", fontSize:"clamp(12px,3.2vw,15px)", fontWeight:850, fontVariantNumeric:"tabular-nums" } },
              React.createElement(BankIcon,{size:15,color:"currentColor"}),
              React.createElement("span",null,L(player.value||0)),
              React.createElement(ComparisonTriangle,{leftValue:isRight?opponentValue:playerValue,rightValue:isRight?playerValue:opponentValue,side,size:12})
            );
            return React.createElement("article", { style:{ position:"relative", minWidth:0, minHeight:205, overflow:"hidden", padding:"17px 13px 15px", borderRadius:22, border:`1px solid color-mix(in srgb, ${overallColor(player.overall)} 48%, var(--border))`, background:`linear-gradient(155deg, color-mix(in srgb, ${overallColor(player.overall)} 19%, var(--surface)) 0%, var(--surface-soft) 52%, var(--surface) 100%)`, boxShadow:`0 18px 42px color-mix(in srgb, ${overallColor(player.overall)} 10%, transparent)`, textAlign:isRight?"right":"left", color:"var(--heading)" } },
              React.createElement("div", { style:{ position:"absolute", width:118, height:118, right:isRight?"auto":-46, left:isRight?-46:"auto", top:-44, borderRadius:999, background:overallColor(player.overall), opacity:.12, filter:"blur(18px)" } }),
              React.createElement("div", { style:{ position:"relative", height:"100%", display:"flex", flexDirection:"column", alignItems:isRight?"flex-end":"flex-start" } },
                overallBlock,
                React.createElement("div", { style:{ width:"100%", display:"flex", flexDirection:"column", alignItems:isRight?"flex-end":"flex-start", marginTop:34, rowGap:4 } },
                  React.createElement("span", { style:{ display:"inline-flex", alignItems:"center", minHeight:23, padding:"4px 8px", borderRadius:8, background:positionColor(player.position), color:"#fff", fontSize:10.5, fontWeight:850 } }, player.position || "—"),
                  React.createElement("div", { style:{ width:"100%", display:"flex", flexDirection:isRight?"row-reverse":"row", alignItems:"flex-start", justifyContent:isRight?"flex-start":"flex-start", gap:5, marginTop:4, minWidth:0 } },
                    React.createElement("strong", { style:{ minWidth:0, fontSize:"clamp(15px,4vw,20px)", lineHeight:1.06, letterSpacing:"-.45px", color:"var(--heading)", overflowWrap:"anywhere", display:"-webkit-box", WebkitLineClamp:2, WebkitBoxOrient:"vertical", overflow:"hidden" } }, player.name || "Jogador"),
                    React.createElement("span", { title:player.nationality||"Nacionalidade não informada", style:{ flex:"0 0 auto", fontSize:15, lineHeight:1.05 } }, nationalityFlag(player.nationality))
                  ),
                  React.createElement("div", { style:{ width:"100%", fontSize:10.5, color:"var(--muted)", lineHeight:1.25, overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" } }, player.club || "Clube não informado"),
                  teamLine && React.createElement("div", { style:{ width:"100%", fontSize:10, color:"var(--green)", fontWeight:800, lineHeight:1.2, overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" } }, teamLine),
                  valueBlock
                )
              )
            );
          ...
          let picker = pickerOpen ? ReactDOM.createPortal(React.createElement("div", { className:"sports-modal-overlay", onClick:()=>setPickerOpen(false), style:{ position:"fixed", inset:0, zIndex:1600, display:"flex", alignItems:"flex-end", justifyContent:"center", background:"rgba(0,0,0,.72)", backdropFilter:"blur(12px)" } },
            React.createElement("div", { onClick:(event)=>event.stopPropagation(), style:{ width:"min(620px,100%)", maxHeight:"82vh", display:"flex", flexDirection:"column", padding:"14px 16px calc(18px + env(safe-area-inset-bottom))", borderRadius:"26px 26px 0 0", border:"1px solid var(--border)", borderBottom:0, background:"var(--surface)", boxShadow:"0 -24px 70px rgba(0,0,0,.35)" } },
              ...
              React.createElement("div", { style:{ display:"flex", alignItems:"center", justifyContent:"space-between", gap:12, marginBottom:14 } },
                React.createElement("div", null, React.createElement("h3", { style:{margin:0,fontSize:20} }, "Comparar com"), React.createElement("div", { style:{fontSize:11.5,color:"var(--muted)",marginTop:3} }, "Busque no mercado e nos elencos")),
                React.createElement("button", { onClick:()=>setPickerOpen(false), "aria-label":"Fechar busca", style:{ width:40,height:40,borderRadius:999,border:"1px solid var(--border)",background:"var(--surface-soft)",color:"var(--heading)",display:"grid",placeItems:"center",cursor:"pointer" } }, React.createElement(Ye,{size:17}))
              ),
              React.createElement("div", { style:{ position:"relative", marginBottom:12 } },
                React.createElement(Xt,{size:17,style:{position:"absolute",left:14,top:"50%",transform:"translateY(-50%)",pointerEvents:"none",color:"var(--muted)"}}),
                React.createElement("input", { autoFocus:true, value:compareQuery, onChange:(event)=>setCompareQuery(event.target.value), placeholder:"Buscar jogador pelo nome", style:{ ...q, width:"100%", minHeight:48, paddingLeft:42, borderRadius:15 } })
              ),
              React.createElement("div", { style:{ overflowY:"auto", display:"grid", gap:8, paddingBottom:4 } },
                compareCandidates.map((player)=>React.createElement("button", { key:player.id, onClick:()=>{setComparePlayer(player);setPickerOpen(false);setCompareQuery("");}, style:{ width:"100%", display:"grid", gridTemplateColumns:"48px minmax(0,1fr) auto", alignItems:"center", gap:12, padding:"10px 12px", borderRadius:15, border:"1px solid var(--border)", background:"var(--surface-soft)", color:"var(--heading)", textAlign:"left", cursor:"pointer" } },
                  React.createElement("div", { style:{ width:48,height:48,borderRadius:13,display:"grid",placeItems:"center",border:`1px solid color-mix(in srgb,${overallColor(player.overall)} 48%,var(--border))`,background:`color-mix(in srgb,${overallColor(player.overall)} 10%,var(--surface))` } }, React.createElement("strong", { style:{fontSize:20,color:overallColor(player.overall)} }, player.overall || "—")),
                  React.createElement("div", { style:{minWidth:0} }, React.createElement("strong", { style:{display:"block",fontSize:14.5,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"} }, player.name), React.createElement("span", { style:{display:"block",fontSize:11,color:"var(--muted)",marginTop:3,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"} }, `${player.position || "—"} · ${sourceLabel(player)}`)),
                  React.createElement(ue,{size:22,color:"var(--muted)"})
                )),
                compareCandidates.length === 0 && React.createElement("div", { style:{padding:"28px 16px",textAlign:"center",color:"var(--muted)",fontSize:13} }, "Nenhum jogador encontrado.")
              )
            )
          ),document.body) : null;
          return React.createElement(
        ...
        function PlayerOverrideHistoryAdmin({ catalog }) {
          let [rows,setRows]=b([]),[loading,setLoading]=b(false),[error,setError]=b("");
          let catalogMap=X(()=>{let map=new Map();(catalog||[]).forEach(player=>player&&map.set(String(player.id),player));return map},[catalog]);
          async function loadHistory(){
            let loader=window.ManchaApp&&window.ManchaApp.loadPlayerOverrideHistory;
            if(typeof loader!=="function"){setError("Execute o SQL e atualize js/supabase.js para carregar o histórico.");return;}
            setLoading(true);setError("");
            try{setRows(await loader(200));}catch(loadError){console.error("override history load failed",loadError);setError("Não foi possível carregar o histórico de alterações.");}finally{setLoading(false);}
          }
          He(()=>{loadHistory()},[]);
          function formatDate(value){return value?new Date(value).toLocaleString("pt-BR",{day:"2-digit",month:"short",year:"numeric",hour:"2-digit",minute:"2-digit"}):"—"}
          function personName(person){return person&&person.nameSnapshot||person&&person.profileId||"Perfil removido"}
          const historyAttributeLabels={jump:"Impulsão",heading:"Cabeceio",topSpeed:"Velocidade máxima",aggression:"Agressividade",acceleration:"Aceleração",agility:"Agilidade",balance:"Equilíbrio",response:"Resposta",technique:"Técnica",dribbleAccuracy:"Precisão do drible",dribbleSpeed:"Velocidade do drible",shotAccuracy:"Precisão do chute",shotPower:"Força do chute",shotTechnique:"Técnica de chute",freeKickAccuracy:"Precisão em cobranças de falta",curve:"Curva",shortPassAccuracy:"Precisão do passe curto",shortPassSpeed:"Velocidade do passe curto",longPassAccuracy:"Precisão do passe longo",longPassSpeed:"Velocidade do passe longo",teamwork:"Trabalho em equipe",stamina:"Resistência",conditionFitness:"Condição física",weakFootAccuracy:"Precisão com o pé ruim",weakFootFrequency:"Frequência de uso do pé ruim",goalKeepingSkills:"Habilidade de goleiro"};
          function dataSummary(row){let before=row.previousData||{},after=row.newData||{},parts=[];[["overall","Overall"],["market_value","Valor"],["attack","Ataque"],["defense","Defesa"]].forEach(([key,label])=>{if(String(before[key]??"")!==String(after[key]??""))parts.push(`${label}: ${before[key]??"—"} → ${after[key]??"—"}`)});let oldAttrs=before.attributes||{},newAttrs=after.attributes||{};Object.keys({...oldAttrs,...newAttrs}).forEach(rawKey=>{let key=String(rawKey).replace(/^attrs\./,"");if(String(oldAttrs[rawKey]??oldAttrs[key]??"")!==String(newAttrs[rawKey]??newAttrs[key]??"")){let oldValue=oldAttrs[rawKey]??oldAttrs[key]??"—",newValue=newAttrs[rawKey]??newAttrs[key]??"—";parts.push(`${historyAttributeLabels[key]||key}: ${oldValue} → ${newValue}`)}});return parts.slice(0,12)}
          function voteList(title,votes){if(!votes||!votes.length)return null;return React.createElement("div",{style:{fontSize:11.5}},React.createElement("div",{style:{fontSize:10,color:"var(--muted)",textTransform:"uppercase",letterSpacing:".06em",marginBottom:4}},title),React.createElement("div",{style:{display:"grid",gap:3}},votes.map((vote,index)=>React.createElement("div",{key:`${title}-${index}`,style:{display:"flex",justifyContent:"space-between",gap:10}},React.createElement("strong",null,personName(vote)),React.createElement("span",{style:{color:"var(--muted)",whiteSpace:"nowrap"}},formatDate(vote.createdAt))))))}
          return React.createElement("div",{style:{marginTop:20,paddingTop:18,borderTop:"1px solid var(--border)"}},
            React.createElement("div",{style:{display:"flex",justifyContent:"space-between",gap:12,alignItems:"center"}},React.createElement("div",null,React.createElement("h3",{style:{margin:"0 0 4px"}},"Histórico de alterações"),React.createElement("div",{style:{fontSize:11.5,color:"var(--muted)"}},"Inclui overrides antigos importados e todas as novas alterações.")),React.createElement("button",{onClick:loadHistory,disabled:loading,style:{...M,padding:"8px 12px",margin:0}},loading?"Atualizando…":"Atualizar")),
            error&&React.createElement("div",{style:{fontSize:12,color:"var(--danger)",marginTop:12}},error),
            !loading&&!error&&!rows.length&&React.createElement("div",{style:{fontSize:12,color:"var(--muted)",padding:"14px 0"}},"Nenhuma alteração registrada."),
            React.createElement("div",{style:{display:"grid",gap:10,marginTop:12,maxHeight:560,overflow:"auto"}},rows.map(row=>{
              let player=catalogMap.get(String(row.playerId)),changes=dataSummary(row),review=row.review,approvals=review&&review.approvals||[],rejections=review&&review.rejections||[],reviewChildren=[];
              if(review){
                reviewChildren.push(React.createElement("div",{key:"author",style:{fontSize:11.5}},React.createElement("div",{style:{fontSize:10,color:"var(--muted)",textTransform:"uppercase",letterSpacing:".06em",marginBottom:3}},"Sugerida por"),React.createElement("strong",null,review.suggestedByNameSnapshot||review.suggestedByProfileId||"Perfil removido"),React.createElement("span",{style:{color:"var(--muted)",marginLeft:6}},"· ",formatDate(review.suggestedAt))));
              }
              return React.createElement("div",{key:row.id,style:{padding:13,borderRadius:14,border:"1px solid var(--border)",background:"var(--surface-soft)"}},
                React.createElement("div",{style:{display:"flex",justifyContent:"space-between",gap:10}},React.createElement("strong",null,player&&player.name||row.playerId),React.createElement("span",{style:{fontSize:10.5,color:"var(--muted)",whiteSpace:"nowrap"}},formatDate(row.createdAt))),
                React.createElement("div",{style:{fontSize:11.5,color:"var(--muted)",marginTop:4}},row.actionLabel||row.action),
                review&&React.createElement("div",{style:{display:"grid",gap:8,marginTop:11,paddingTop:10,borderTop:"1px solid var(--border)"}},reviewChildren),
                !review&&React.createElement("div",{style:{fontSize:11.5,color:"var(--muted)",marginTop:6}},row.actorNameSnapshot||row.actorProfileId||"Sistema"),
                changes.length?React.createElement("div",{style:{display:"grid",gap:4,marginTop:9,fontSize:11.5,paddingTop:review?9:0,borderTop:review?"1px solid var(--border)":"none"}},changes.map((item,index)=>React.createElement("div",{key:index},item))):null,
                review&&React.createElement("div",{style:{display:"grid",gap:8,marginTop:10,paddingTop:10,borderTop:"1px solid var(--border)"}},(()=>{let nodes=[];let approved=voteList("Aprovada por",approvals);if(approved)nodes.push(approved);let rejected=voteList("Recusada por",rejections);if(rejected)nodes.push(rejected);if(review.resolvedAt)nodes.push(React.createElement("div",{key:"resolved",style:{fontSize:10.5,color:"var(--muted)"}},"Concluída em ",formatDate(review.resolvedAt)));return nodes})()),
                row.metadata&&row.metadata.importedFromLegacy&&React.createElement("div",{style:{fontSize:10.5,color:"var(--muted)",marginTop:8}},review?"Registro antigo relacionado à revisão e aos votos preservados.":"Registro reconstruído a partir de um override anterior ao histórico.")
              );
            }))
          );
        }
        function MarketAccessAdminForm({ tournament, onSave }) {
          ...
          let minimumGames=settings.minimumGamesForMarket&&typeof settings.minimumGamesForMarket==="object"?settings.minimumGamesForMarket:{};
          let releaseClause=settings.releaseClause&&typeof settings.releaseClause==="object"?settings.releaseClause:{};
          let [isOpen,setIsOpen]=b(settings.isOpen!==false),[reportsEnabled,setReportsEnabled]=b(settings.playerReportsEnabled!==false),[overridesEnabled,setOverridesEnabled]=b(settings.playerOverridesEnabled!==false),[limitEnabled,setLimitEnabled]=b(limit.enabled===true),[minOverall,setMinOverall]=b(limit.minOverall!=null?limit.minOverall:1),[maxOverall,setMaxOverall]=b(limit.maxOverall!=null?limit.maxOverall:99),[tradeLockEnabled,setTradeLockEnabled]=b(tradeLock.enabled===true),[tradeLockGames,setTradeLockGames]=b(tradeLock.gamesRequired!=null?tradeLock.gamesRequired:50),[minimumGamesEnabled,setMinimumGamesEnabled]=b(minimumGames.enabled===true),[minimumGamesRequired,setMinimumGamesRequired]=b(minimumGames.gamesRequired!=null?minimumGames.gamesRequired:0),[rcEnabled,setRcEnabled]=b(releaseClause.enabled===true),[rcMultiplier,setRcMultiplier]=b(releaseClause.multiplier!=null?releaseClause.multiplier:2),[rcRevalue,setRcRevalue]=b(releaseClause.revalueMultiplier!=null?releaseClause.revalueMultiplier:1.5),[rcProtectedGames,setRcProtectedGames]=b(releaseClause.protectedGames!=null?releaseClause.protectedGames:50),[rcMaxSuffered,setRcMaxSuffered]=b(releaseClause.maxSufferedPerTeam!=null?releaseClause.maxSufferedPerTeam:2),[rcRepurchase,setRcRepurchase]=b(releaseClause.blockDirectRepurchase!==false),[rcShielding,setRcShielding]=b(releaseClause.shieldingEnabled!==false),[rcShieldConversion,setRcShieldConversion]=b(releaseClause.shieldingConversion!=null?releaseClause.shieldingConversion:1),[rcShieldMax,setRcShieldMax]=b(releaseClause.shieldingMaxPct!=null?releaseClause.shieldingMaxPct:100);
          He(()=>{setIsOpen(settings.isOpen!==false);setReportsEnabled(settings.playerReportsEnabled!==false);setOverridesEnabled(settings.playerOverridesEnabled!==false);setLimitEnabled(limit.enabled===true);setMinOverall(limit.minOverall!=null?limit.minOverall:1);setMaxOverall(limit.maxOverall!=null?limit.maxOverall:99);setTradeLockEnabled(tradeLock.enabled===true);setTradeLockGames(tradeLock.gamesRequired!=null?tradeLock.gamesRequired:50);setMinimumGamesEnabled(minimumGames.enabled===true);setMinimumGamesRequired(minimumGames.gamesRequired!=null?minimumGames.gamesRequired:0);setRcEnabled(releaseClause.enabled===true);setRcMultiplier(releaseClause.multiplier!=null?releaseClause.multiplier:2);setRcRevalue(releaseClause.revalueMultiplier!=null?releaseClause.revalueMultiplier:1.5);setRcProtectedGames(releaseClause.protectedGames!=null?releaseClause.protectedGames:50);setRcMaxSuffered(releaseClause.maxSufferedPerTeam!=null?releaseClause.maxSufferedPerTeam:2);setRcRepurchase(releaseClause.blockDirectRepurchase!==false);setRcShielding(releaseClause.shieldingEnabled!==false);setRcShieldConversion(releaseClause.shieldingConversion!=null?releaseClause.shieldingConversion:1);setRcShieldMax(releaseClause.shieldingMaxPct!=null?releaseClause.shieldingMaxPct:100)},[tournament&&tournament.id,settings.isOpen,settings.playerReportsEnabled,settings.playerOverridesEnabled,limit.enabled,limit.minOverall,limit.maxOverall,tradeLock.enabled,tradeLock.gamesRequired,minimumGames.enabled,minimumGames.gamesRequired,releaseClause.enabled,releaseClause.multiplier,releaseClause.revalueMultiplier,releaseClause.protectedGames,releaseClause.maxSufferedPerTeam,releaseClause.blockDirectRepurchase,releaseClause.shieldingEnabled,releaseClause.shieldingConversion,releaseClause.shieldingMaxPct]);
          return React.createElement("div",{style:{marginTop:18,paddingTop:18,borderTop:"1px solid var(--border)"}},
            ...
            React.createElement("label",{style:{display:"flex",justifyContent:"space-between",alignItems:"center",gap:12,padding:"12px 0"}},React.createElement("span",null,React.createElement("strong",null,"Aplicar overrides"),React.createElement("div",{style:{fontSize:11.5,color:"var(--muted)",marginTop:3,lineHeight:1.4}},"Quando desligado, o campeonato usa os dados originais do catálogo. As correções aprovadas permanecem salvas e voltam a valer ao reativar.")),React.createElement("input",{type:"checkbox",checked:overridesEnabled,onChange:(event)=>setOverridesEnabled(event.target.checked)})),
            ...
            React.createElement("label",{style:{display:"flex",justifyContent:"space-between",alignItems:"center",gap:12,padding:"12px 0"}},React.createElement("span",null,React.createElement("strong",null,"Exigir jogos para negociar"),React.createElement("div",{style:{fontSize:11.5,color:"var(--muted)",marginTop:3,lineHeight:1.4}},"Bloqueia compras, vendas, ofertas, contrapropostas e aceitações até o time atingir o mínimo de partidas jogadas no campeonato.")),React.createElement("input",{type:"checkbox",checked:minimumGamesEnabled,onChange:(event)=>setMinimumGamesEnabled(event.target.checked)})),
            minimumGamesEnabled&&React.createElement("div",null,React.createElement("label",{style:P},"Mínimo de jogos do usuário"),React.createElement("input",{type:"number",min:1,max:999,step:1,value:minimumGamesRequired,onChange:(event)=>setMinimumGamesRequired(event.target.value),style:q}),React.createElement("div",{style:{fontSize:11.5,color:"var(--muted)",marginTop:7,lineHeight:1.4}},"O usuário continua podendo visualizar o mercado, mas só negocia depois que seu time disputar essa quantidade de partidas.")),
            React.createElement("div",{style:{height:1,background:"var(--border)",margin:"18px 0"}}),
            ...
            React.createElement("div",{style:{height:1,background:"var(--border)",margin:"18px 0"}}),
            React.createElement("h4",{style:{margin:"0 0 4px",fontSize:14}},"Multa rescisória"),
            React.createElement("div",{style:{fontSize:11.5,color:"var(--muted)",lineHeight:1.45,marginBottom:8}},"Permite contratar jogadores de outros times pagando uma multa maior que o valor de mercado. O dono pode gastar saldo para blindar a multa."),
            React.createElement("label",{style:{display:"flex",justifyContent:"space-between",alignItems:"center",gap:12,padding:"10px 0"}},React.createElement("strong",null,"Ativar multa rescisória"),React.createElement("input",{type:"checkbox",checked:rcEnabled,onChange:e=>setRcEnabled(e.target.checked)})),
            rcEnabled&&React.createElement(React.Fragment,null,
              React.createElement("div",{style:{display:"grid",gridTemplateColumns:"repeat(2,minmax(0,1fr))",gap:10}},
                React.createElement("div",null,React.createElement("label",{style:P},"Preço da multa (× valor)"),React.createElement("input",{type:"number",min:1,step:.1,value:rcMultiplier,onChange:e=>setRcMultiplier(e.target.value),style:q})),
                React.createElement("div",null,React.createElement("label",{style:P},"Novo valor (× valor anterior)"),React.createElement("input",{type:"number",min:1,step:.1,value:rcRevalue,onChange:e=>setRcRevalue(e.target.value),style:q})),
                React.createElement("div",null,React.createElement("label",{style:P},"Contrato protegido (jogos)"),React.createElement("input",{type:"number",min:0,step:1,value:rcProtectedGames,onChange:e=>setRcProtectedGames(e.target.value),style:q})),
                React.createElement("div",null,React.createElement("label",{style:P},"Máximo sofrido por time"),React.createElement("input",{type:"number",min:0,step:1,value:rcMaxSuffered,onChange:e=>setRcMaxSuffered(e.target.value),style:q}))
              ),
              React.createElement("label",{style:{display:"flex",justifyContent:"space-between",alignItems:"center",gap:12,padding:"10px 0"}},React.createElement("span",null,React.createElement("strong",null,"Bloquear recompra direta"),React.createElement("div",{style:{fontSize:11.5,color:"var(--muted)",marginTop:3}},"O antigo dono não pode recomprar diretamente do time que levou o jogador.")),React.createElement("input",{type:"checkbox",checked:rcRepurchase,onChange:e=>setRcRepurchase(e.target.checked)})),
              React.createElement("label",{style:{display:"flex",justifyContent:"space-between",alignItems:"center",gap:12,padding:"10px 0"}},React.createElement("span",null,React.createElement("strong",null,"Permitir blindagem"),React.createElement("div",{style:{fontSize:11.5,color:"var(--muted)",marginTop:3}},"O dono pode gastar saldo para aumentar a multa de um jogador.")),React.createElement("input",{type:"checkbox",checked:rcShielding,onChange:e=>setRcShielding(e.target.checked)})),
              rcShielding&&React.createElement("div",{style:{display:"grid",gridTemplateColumns:"repeat(2,minmax(0,1fr))",gap:10}},
                React.createElement("div",null,React.createElement("label",{style:P},"Conversão da blindagem"),React.createElement("input",{type:"number",min:.01,step:.1,value:rcShieldConversion,onChange:e=>setRcShieldConversion(e.target.value),style:q}),React.createElement("div",{style:{fontSize:10.5,color:"var(--muted)",marginTop:5}},"1M gasto adiciona este valor à multa")),
                React.createElement("div",null,React.createElement("label",{style:P},"Blindagem máxima (% da base)"),React.createElement("input",{type:"number",min:0,step:5,value:rcShieldMax,onChange:e=>setRcShieldMax(e.target.value),style:q}))
              )
            ),
            React.createElement("button",{onClick:()=>onSave(isOpen,limitEnabled,minOverall,maxOverall,tradeLockEnabled,tradeLockGames,reportsEnabled,overridesEnabled,minimumGamesEnabled,minimumGamesRequired,{enabled:rcEnabled,multiplier:rcMultiplier,revalueMultiplier:rcRevalue,protectedGames:rcProtectedGames,maxSufferedPerTeam:rcMaxSuffered,blockDirectRepurchase:rcRepurchase,shieldingEnabled:rcShielding,shieldingConversion:rcShieldConversion,shieldingMaxPct:rcShieldMax}),disabled:(limitEnabled&&Number(minOverall)>Number(maxOverall))||(minimumGamesEnabled&&Number(minimumGamesRequired)<1),style:{...M,...W,marginTop:12,opacity:(limitEnabled&&Number(minOverall)>Number(maxOverall))||(minimumGamesEnabled&&Number(minimumGamesRequired)<1)?.55:1}},"Salvar regras do mercado")
          );
        }
        ...
        function ProfileArea({ profile, team, tournament, squad, stats, matches, transfers, theme, setTheme, onUpdateProfile, onUpdateTeamName, onSwitchProfile, onOpenBalanceHistory, tournaments, onOpenChampionshipSummary }) {
          ...
          let allTournaments=Array.isArray(tournaments)?tournaments:[];
          function tournamentTeamForProfile(comp){ let contextTeams=Array.isArray(comp&&comp.context&&comp.context.teams)?comp.context.teams:[]; return contextTeams.find((entry)=>entry&&entry.active!==false&&String(entry.profileId)===String(item.id))||null; }
          let careerMatches=[];
          allTournaments.forEach((comp)=>{ let compTeam=tournamentTeamForProfile(comp); if(!compTeam)return; let compMatches=Array.isArray(comp&&comp.matches)?comp.matches:Array.isArray(comp&&comp.context&&comp.context.matches)?comp.context.matches:[]; compMatches.forEach((match)=>{ if(!match||!match.played||match.bye||match.status==="voided")return; if(String(match.homeId)!==String(compTeam.id)&&String(match.awayId)!==String(compTeam.id))return; let home=String(match.homeId)===String(compTeam.id),gf=Number(home?match.homeScore:match.awayScore)||0,ga=Number(home?match.awayScore:match.homeScore)||0; careerMatches.push({match,comp,gf,ga,result:gf>ga?"V":gf<ga?"D":"E",timestamp:Number(match.playedAt||match.createdAt||comp.createdAt)||0}); }); });
          if(tournament&&team){ let known=new Set(careerMatches.map((entry)=>`${entry.comp&&entry.comp.id}:${entry.match&&entry.match.id}`)); (Array.isArray(matches)?matches:[]).forEach((match)=>{ if(!match||!match.played||match.bye||match.status==="voided"||(String(match.homeId)!==String(team.id)&&String(match.awayId)!==String(team.id)))return; let key=`${tournament.id}:${match.id}`; if(known.has(key))return; let home=String(match.homeId)===String(team.id),gf=Number(home?match.homeScore:match.awayScore)||0,ga=Number(home?match.awayScore:match.homeScore)||0; careerMatches.push({match,comp:tournament,gf,ga,result:gf>ga?"V":gf<ga?"D":"E",timestamp:Number(match.playedAt||match.createdAt||tournament.createdAt)||0}); }); }
          careerMatches.sort((a,b)=>a.timestamp-b.timestamp);
          let careerWins=0,careerDraws=0,careerLosses=0,careerGoals=0,careerAgainst=0,longestWins=0,streak=0;
          careerMatches.forEach((entry)=>{careerGoals+=entry.gf;careerAgainst+=entry.ga;if(entry.result==="V"){careerWins++;streak++;longestWins=Math.max(longestWins,streak)}else{streak=0;entry.result==="E"?careerDraws++:careerLosses++;}});
          let careerGames=careerMatches.length,careerGpg=careerGames?careerGoals/careerGames:0,careerPct=careerGames?((careerWins*3+careerDraws)/(careerGames*3))*100:0;
          let careerTransactions=0,careerSpent=0;
          allTournaments.forEach((comp)=>{let compTeam=tournamentTeamForProfile(comp);if(!compTeam)return;let txs=Array.isArray(comp&&comp.context&&comp.context.transfers)?comp.context.transfers:[];txs.filter((tx)=>tx&&!tx.rolledBackAt&&(String(tx.fromTeamId||"")===String(compTeam.id)||String(tx.toTeamId||"")===String(compTeam.id))&&["market_purchase","market_sale","user_transfer","release_clause"].includes(String(tx.type||""))).forEach((tx)=>{careerTransactions++;if(String(tx.toTeamId||"")===String(compTeam.id))careerSpent+=Math.abs(Number(tx.price)||0);});});
          let careerRecent=careerMatches.slice().sort((a,b)=>b.timestamp-a.timestamp).slice(0,6).reverse();
          let careerTrophies=allTournaments.filter((comp)=>comp&&comp.status==="finished").map((comp)=>{let standings=Array.isArray(comp.finalStandings)?comp.finalStandings:[];let entry=standings.find((row)=>row&&String(row.profileId)===String(item.id)&&Number(row.position)===1);return entry?{tournament:comp,entry}:null;}).filter(Boolean).sort((a,b)=>(Number(b.tournament.finishedAt)||0)-(Number(a.tournament.finishedAt)||0));
          function profileResultSegment(value,pct,label,kind){let bg=kind==="win"?"color-mix(in srgb,var(--green) 18%,var(--surface-soft))":kind==="loss"?"color-mix(in srgb,var(--danger) 14%,var(--surface-soft))":"var(--surface-soft)";let color=kind==="win"?"var(--green)":kind==="loss"?"var(--danger)":"var(--heading)";return React.createElement("div",{style:{flex:`${Math.max(pct,18)} 1 0`,minWidth:0,padding:"14px 8px",background:bg,textAlign:"center",borderRadius:12}},React.createElement("div",{style:{fontSize:20,fontWeight:900,fontVariantNumeric:"tabular-nums",lineHeight:1,color}},value),React.createElement("div",{style:{fontSize:10.5,fontWeight:800,marginTop:7,color}},label),React.createElement("div",{style:{fontSize:10.5,color:"var(--muted)",marginTop:3}},`${Math.round(pct)}%`));}
          function profileStatRow(label,value,last=false){return React.createElement("div",{style:{display:"flex",alignItems:"center",justifyContent:"space-between",gap:18,padding:"13px 0",borderBottom:last?0:"1px solid var(--border)"}},React.createElement("span",{style:{fontSize:12.5,color:"var(--muted)",fontWeight:700}},label),React.createElement("strong",{style:{fontSize:15,fontVariantNumeric:"tabular-nums",textAlign:"right"}},value));}
          function profileResultBadge(entry,index,isLatest){let result=entry.result,badge=React.createElement("span",{style:{width:30,height:30,borderRadius:9,display:"grid",placeItems:"center",fontWeight:850,fontSize:12,background:result==="V"?"rgba(0,201,120,.18)":result==="D"?"rgba(255,43,58,.18)":"rgba(255,187,38,.18)",color:result==="V"?"#00c978":result==="D"?"#ff2b3a":"#ffbb26"}},result);return isLatest?React.createElement("span",{key:index,style:{display:"inline-grid",placeItems:"center",padding:1,border:"1px solid var(--heading)",borderRadius:11}},badge):React.createElement("span",{key:index},badge);}
          ...
          return React.createElement("div", null,
            ...
            team && React.createElement("section",{className:"family-card",style:{padding:"18px 20px",marginBottom:16}},
              React.createElement("div",{style:{display:"grid",gridTemplateColumns:"repeat(2,minmax(0,1fr))",gap:12}},
                React.createElement("button",{type:"button",onClick:onOpenBalanceHistory,className:"tapbtn",style:{border:0,background:"var(--surface-soft)",borderRadius:14,padding:14,textAlign:"left",color:"inherit",cursor:"pointer"}},React.createElement("div",{style:{fontSize:11.5,color:"var(--muted)",marginBottom:5}},"Saldo"),React.createElement("strong",{style:{fontSize:18}},L(team.budget||0))),
                React.createElement("div",{style:{background:"var(--surface-soft)",borderRadius:14,padding:14}},React.createElement("div",{style:{fontSize:11.5,color:"var(--muted)",marginBottom:5}},"Elenco atual"),React.createElement("strong",{style:{fontSize:18}},`${squad.length} jogadores`))
              )
            ),
            React.createElement("section",{className:"family-card",style:{padding:"18px 20px",marginBottom:16}},
              React.createElement("div",{style:{display:"flex",alignItems:"baseline",justifyContent:"space-between",gap:12,marginBottom:14}},React.createElement("div",{style:{fontSize:17,fontWeight:850}},"Estatísticas gerais"),React.createElement("div",{style:{fontSize:11,color:"var(--muted)"}},"Todos os campeonatos")),
              careerGames?React.createElement(React.Fragment,null,
                React.createElement("div",{style:{fontSize:12,color:"var(--muted)",textAlign:"center",marginBottom:10}},`${careerGames} ${careerGames===1?"partida":"partidas"}`),
                React.createElement("div",{style:{display:"flex",gap:5,alignItems:"stretch",marginBottom:14}},profileResultSegment(careerWins,(careerWins/careerGames)*100,"Vitórias","win"),profileResultSegment(careerDraws,(careerDraws/careerGames)*100,"Empates","draw"),profileResultSegment(careerLosses,(careerLosses/careerGames)*100,"Derrotas","loss")),
                profileStatRow("Gols",careerGoals),profileStatRow("Gols por jogo",careerGpg.toLocaleString("pt-BR",{minimumFractionDigits:2,maximumFractionDigits:2})),profileStatRow("Aproveitamento",`${Math.round(careerPct)}%`),profileStatRow("Maior sequência de vitórias",longestWins),profileStatRow("Transações",careerTransactions),profileStatRow("Valor gasto",L(careerSpent),true)
              ):React.createElement("div",{style:{padding:"18px 0 4px",textAlign:"center",fontSize:13,color:"var(--muted)"}},"As estatísticas aparecem após a primeira partida.")
            ),
            React.createElement("section",{className:"family-card",style:{padding:"18px 20px",marginBottom:16}},
              React.createElement("div",{style:{fontSize:14,fontWeight:850,marginBottom:12}},"Últimas partidas"),
              careerRecent.length?React.createElement("div",{style:{display:"flex",gap:8,alignItems:"center",flexWrap:"wrap"}},careerRecent.map((entry,index)=>profileResultBadge(entry,index,index===careerRecent.length-1))):React.createElement("div",{style:{fontSize:12.5,color:"var(--muted)"}},"Nenhuma partida disputada ainda.")
            ),
            React.createElement("section",{className:"family-card",style:{padding:"18px 20px",marginBottom:18}},
              React.createElement("div",{style:{fontSize:17,fontWeight:850,marginBottom:14}},"Troféus conquistados"),
              careerTrophies.length?React.createElement("div",{style:{display:"grid",gridTemplateColumns:"repeat(2,minmax(0,1fr))",gap:10}},careerTrophies.map(({tournament:comp})=>React.createElement("button",{key:comp.id,onClick:()=>onOpenChampionshipSummary&&onOpenChampionshipSummary(comp),className:"tapbtn",style:{padding:16,borderRadius:16,border:"1px solid var(--border)",background:"var(--surface-soft)",textAlign:"center",cursor:"pointer",color:"inherit"}},React.createElement(TrophyAsset,{tournament:comp,size:74,style:{marginBottom:8}}),React.createElement("div",{style:{fontWeight:800,fontSize:13}},comp.name),React.createElement("div",{style:{color:"var(--muted)",fontSize:11.5,marginTop:4}},comp.type==="cup"?"Campeão da Copa":"Campeão")))):React.createElement("div",{style:{padding:"18px 4px",textAlign:"center",color:"var(--muted)",fontSize:12.5,lineHeight:1.5}},"Nenhum troféu conquistado ainda.")
            ),
        ...
        function recomputeSingleRosterPreview(entries, catalog) {
          let seen=new Map();
          let normalized=(entries||[]).map((entry)=>({...entry}));
          normalized.forEach((entry)=>{
            if(!entry.playerId||entry.status==="owned")return;
            if(seen.has(entry.playerId)){
              if(entry.status!=="replacement")entry.status="duplicate";
              let previous=normalized.find((candidate)=>candidate!==entry&&candidate.playerId===entry.playerId&&candidate.status!=="owned");
              if(previous&&previous.status!=="replacement")previous.status="duplicate";
            }else seen.set(entry.playerId,true);
          });
          let usableEntries=normalized.filter((entry)=>entry.playerId&&(entry.status==="matched"||entry.status==="replacement"));
          let ignoredOwnedEntries=normalized.filter((entry)=>entry.status==="owned");
          let matchedPlayers=usableEntries.map((entry)=>(catalog||[]).find((player)=>String(player.id)===String(entry.playerId))).filter(Boolean);
          ...
          return {entries:normalized,total:normalized.length,matchedEntries:usableEntries,ignoredOwnedEntries,ignoredOwnedCount:ignoredOwnedEntries.length,usableCount:usableEntries.length,matchedPlayers,metrics:balancedRosterMetrics(matchedPlayers),groupCounts};
        }

        function equivalentReplacementForEntry(entry, entries, catalog, ownership) {
          if(!entry||!entry.playerId)return null;
          let original=(catalog||[]).find((player)=>String(player.id)===String(entry.playerId));
          if(!original)return null;
          let originalPosition=String(original.position||"").toUpperCase();
          let originalOverall=Number(original.overall)||0;
          let originalValue=Number(original.value)||0;
          let originalGroup=balancedRosterGroupFor(original);
          let used=new Set((entries||[]).filter((item)=>item!==entry&&item.playerId&&(item.status==="matched"||item.status==="replacement")).map((item)=>String(item.playerId)));
          let available=(catalog||[]).filter((player)=>{
            if(!player||player.id==null||String(player.id)===String(original.id)||used.has(String(player.id)))return false;
            let owner=ownership&&ownership[String(player.id)];
            return !(owner&&owner.teamId);
          });
          function tier(player){
            let pos=String(player.position||"").toUpperCase(),ovr=Number(player.overall)||0,delta=Math.abs(ovr-originalOverall),group=balancedRosterGroupFor(player);
            if(pos===originalPosition&&delta===0)return 0;
            if(pos===originalPosition&&delta<=1)return 1;
            if(originalGroup&&group&&group.key===originalGroup.key&&delta===0)return 2;
            if(originalGroup&&group&&group.key===originalGroup.key&&delta<=1)return 3;
            return 99;
          }
          let candidates=available.map((player)=>({player,tier:tier(player),valueDistance:Math.abs((Number(player.value)||0)-originalValue)})).filter((item)=>item.tier<99);
          if(!candidates.length)return null;
          let bestTier=Math.min(...candidates.map((item)=>item.tier));
          let pool=candidates.filter((item)=>item.tier===bestTier).sort((a,b)=>a.valueDistance-b.valueDistance);
          let closest=pool.slice(0,Math.min(5,pool.length));
          let picked=closest[Math.floor(Math.random()*closest.length)]||pool[0];
          return picked?{player:picked.player,tier:picked.tier,original}:null;
        }
        ...
        function parseSingleRosterTxt(text, catalog, ownership) {
          ...
          return recomputeSingleRosterPreview(entries,catalog);
        }
...
Hyago 0 x 0 Lucas`;
        ...
        function AdminArea({ currentTournament, tournaments, teams, profiles, profileName, setProfileName, profileColor, setProfileColor, profileBudget, setProfileBudget, tournamentName, setTournamentName, onCreateProfile, onCreateTournament, onDeleteTournament, onSelectTournament, onUpdateBudget, onToggleParticipant, onPrepareLateJoin, onImportLateJoinTxt, onDeleteProfile, onResetTournament, onRemoveOrphanParticipant, onRestoreOrphanProfile, onUpdateMarketDepreciation, onUpdateInitialRosterDepreciation, onUpdateMarketBalanceRules, onUpdateMarketAccessRules, onUpdateRosterRules, onUpdateEconomyRules, onUpdateBalanceLoanSettings, onGrantBalanceLoan, onFinishTournament, catalog, onImportRosters, onImportHistoricalCompetition, onImportMissingMatches }) {
          ...
          function resolveLateJoinTxtEntry(index,playerId){setLateJoinTxtPreview((current)=>{if(!current)return current;let ownership=currentTournament&&currentTournament.context&&currentTournament.context.ownership||{};let entries=current.entries.map((entry,i)=>i===index?{...entry,playerId:playerId||null,status:playerId?(ownership[String(playerId)]?.teamId?"owned":"matched"):"not_found",replacementOf:null,replacementTier:null}:{...entry});return {...current,...recomputeSingleRosterPreview(entries,catalog)};});}
          function replaceLateJoinTxtEntry(index){let replaced=false;setLateJoinTxtPreview((current)=>{if(!current)return current;let ownership=currentTournament&&currentTournament.context&&currentTournament.context.ownership||{};let entries=current.entries.map((entry)=>({...entry})),entry=entries[index];if(!entry||entry.status!=="owned")return current;let picked=equivalentReplacementForEntry(entry,entries,catalog,ownership);if(!picked)return current;entries[index]={...entry,playerId:String(picked.player.id),status:"replacement",replacementOf:{playerId:String(picked.original.id),name:picked.original.name,position:picked.original.position,overall:picked.original.overall,club:picked.original.club,value:picked.original.value},replacementTier:picked.tier};replaced=true;return {...current,...recomputeSingleRosterPreview(entries,catalog)};});if(!replaced)window.alert("Não encontrei um jogador livre suficientemente equivalente para esta posição e overall.");}
          function replaceAllLateJoinTxtOwned(){let replacedCount=0,missingCount=0;setLateJoinTxtPreview((current)=>{if(!current)return current;let ownership=currentTournament&&currentTournament.context&&currentTournament.context.ownership||{};let entries=current.entries.map((entry)=>({...entry}));for(let index=0;index<entries.length;index++){let entry=entries[index];if(entry.status!=="owned")continue;let picked=equivalentReplacementForEntry(entry,entries,catalog,ownership);if(!picked){missingCount++;continue;}entries[index]={...entry,playerId:String(picked.player.id),status:"replacement",replacementOf:{playerId:String(picked.original.id),name:picked.original.name,position:picked.original.position,overall:picked.original.overall,club:picked.original.club,value:picked.original.value},replacementTier:picked.tier};replacedCount++;}return {...current,...recomputeSingleRosterPreview(entries,catalog)};});if(!replacedCount&&missingCount)window.alert("Não encontrei substitutos equivalentes disponíveis para estes jogadores.");else if(missingCount)window.alert(`${replacedCount} substituição${replacedCount===1?" realizada":"ões realizadas"}. ${missingCount} jogador${missingCount===1?" continua":"es continuam"} sem equivalente disponível e ${missingCount===1?"poderá":"poderão"} ser ignorado${missingCount===1?"":"s"}.`);}
          async function confirmLateJoinTxt(){if(!lateJoinProfileId||lateJoinBusy||!lateJoinTxtPreview)return;let blockers=lateJoinTxtPreview.entries.filter((entry)=>entry.status!=="matched"&&entry.status!=="replacement"&&entry.status!=="owned"),usableEntries=lateJoinTxtPreview.entries.filter((entry)=>entry.status==="matched"||entry.status==="replacement"),ignoredOwned=lateJoinTxtPreview.entries.filter((entry)=>entry.status==="owned"),replacements=lateJoinTxtPreview.entries.filter((entry)=>entry.status==="replacement");if(blockers.length){window.alert("Resolva as pendências do TXT antes de confirmar. Jogadores que já pertencem a outro time podem ser substituídos por equivalentes ou ignorados.");return;}if(!usableEntries.length){window.alert("Nenhum jogador disponível restou para importar.");return;}let message=`Confirmar a entrada com ${usableEntries.length} jogador${usableEntries.length===1?"":"es"}?`;if(replacements.length)message+=`

${replacements.length} jogador${replacements.length===1?" foi substituído":"es foram substituídos"} por equivalente${replacements.length===1?"":"s"}.`;if(ignoredOwned.length)message+=`

${ignoredOwned.length} jogador${ignoredOwned.length===1?" já pertence":"es já pertencem"} a outro time e ${ignoredOwned.length===1?"será ignorado":"serão ignorados"}.`;if(usableEntries.length<23)message+=`

O elenco ficará abaixo de 23 jogadores e poderá ser completado depois.`;if(!window.confirm(message))return;setLateJoinBusy(true);try{await onImportLateJoinTxt({profileId:lateJoinProfileId,budget:lateJoinBudget,entries:usableEntries.map((entry)=>({playerId:entry.playerId,squadRole:entry.squadRole}))});setLateJoinOpen(false);setLateJoinTxt("");setLateJoinTxtPreview(null);window.alert(`Participante adicionado com ${usableEntries.length} jogador${usableEntries.length===1?"":"es"}.${replacements.length?` ${replacements.length} substituição${replacements.length===1?" foi aplicada":"ões foram aplicadas"}.`:""}${ignoredOwned.length?` ${ignoredOwned.length} indisponível${ignoredOwned.length===1?" foi ignorado":"is foram ignorados"}.`:""}`);}catch(error){console.error("late join txt import failed",error);window.alert(error&&error.message?error.message:"Não foi possível adicionar o participante pelo TXT.");}finally{setLateJoinBusy(false);}}
          ...
          return React.createElement("div", null,
            ...
            adminSection === "participants" && currentTournament && currentTournament.type!=="cup" && currentTournament.status==="ongoing" && currentTournament.randomRoster&&currentTournament.randomRoster.enabled&&currentTournament.randomRoster.status==="active" && React.createElement("div", { style:{...E,border:"1px solid color-mix(in srgb,var(--green) 34%,var(--border))"} },
              ...
              lateJoinOpen&&React.createElement("div",{style:{marginTop:16,paddingTop:16,borderTop:"1px solid var(--border)",display:"grid",gap:12}},
                ...
                lateJoinMode==="txt"&&React.createElement(React.Fragment,null,
                  ...
                  React.createElement("textarea",{value:lateJoinTxt,onChange:(event)=>{setLateJoinTxt(event.target.value);setLateJoinTxtPreview(null);},placeholder:"Cole os jogadores, um por linha. Recomendado: Nome | Time",rows:7,style:{...q,resize:"vertical",minHeight:140,fontFamily:"ui-monospace,SFMono-Regular,Menlo,monospace",fontSize:11.5}}),
                  ...
                  lateJoinTxtPreview&&React.createElement("div",{style:{display:"grid",gap:10}},
                    React.createElement("div",{style:{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(92px,1fr))",gap:8}},[["No TXT",String(lateJoinTxtPreview.total||0)],["Aproveitados",String(lateJoinTxtPreview.usableCount!=null?lateJoinTxtPreview.usableCount:lateJoinTxtPreview.matchedPlayers.length)],["Ignorados",String(lateJoinTxtPreview.ignoredOwnedCount||0)],["OVR médio",lateJoinTxtPreview.matchedPlayers.length?Number(lateJoinTxtPreview.metrics.averageOverall||0).toFixed(1):"—"],["Valor",lateJoinTxtPreview.matchedPlayers.length?`${Math.round(Number(lateJoinTxtPreview.metrics.marketValue)||0)}M`:"—"]].map(([label,value])=>React.createElement("div",{key:label,style:{padding:10,borderRadius:12,background:"var(--surface-soft)",textAlign:"center"}},React.createElement("div",{style:{fontSize:10.5,color:"var(--muted)"}},label),React.createElement("strong",{style:{display:"block",marginTop:3}},value)))),
                    ...
                    (lateJoinTxtPreview.ignoredOwnedCount||0)>0&&React.createElement("div",{style:{display:"grid",gap:8,padding:10,borderRadius:10,background:"color-mix(in srgb,#ffbb26 10%,var(--surface))",border:"1px solid color-mix(in srgb,#ffbb26 32%,var(--border))",fontSize:11.5,lineHeight:1.45}},React.createElement("div",null,`${lateJoinTxtPreview.ignoredOwnedCount} jogador${lateJoinTxtPreview.ignoredOwnedCount===1?" do TXT já pertence":"es do TXT já pertencem"} a outro time. Você pode ignorar ${lateJoinTxtPreview.ignoredOwnedCount===1?"esse jogador":"esses jogadores"} ou tentar preencher as vagas com equivalentes livres.`),React.createElement("button",{type:"button",onClick:replaceAllLateJoinTxtOwned,style:{padding:"8px 10px",borderRadius:9,border:"1px solid color-mix(in srgb,#ffbb26 45%,var(--border))",background:"var(--surface)",color:"var(--heading)",fontWeight:800,cursor:"pointer",justifySelf:"start"}},`Substituir ${lateJoinTxtPreview.ignoredOwnedCount===1?"jogador":"todos"} automaticamente`)),
                    lateJoinTxtPreview.matchedPlayers.length>0&&Math.abs(Number(lateJoinTxtPreview.metrics.averageOverall||0)-Number(lateJoinBenchmark&&lateJoinBenchmark.averageOverall||0))>1&&React.createElement("div",{style:{padding:10,borderRadius:10,background:"color-mix(in srgb,#ffbb26 12%,var(--surface))",border:"1px solid color-mix(in srgb,#ffbb26 35%,var(--border))",fontSize:11.5,lineHeight:1.45}},"Atenção: o overall médio deste elenco está mais de 1 ponto distante da referência atual do campeonato. O admin ainda pode confirmar manualmente."),
                    React.createElement("div",{style:{maxHeight:340,overflow:"auto",borderTop:"1px solid var(--border)"}},lateJoinTxtPreview.entries.map((entry,index)=>{let matched=entry.playerId?(catalog||[]).find((player)=>String(player.id)===String(entry.playerId)):null;let replacement=entry.status==="replacement";return React.createElement("div",{key:index,style:{padding:"9px 0",borderBottom:"1px solid var(--border)",display:"grid",gap:6}},React.createElement("div",{style:{display:"flex",justifyContent:"space-between",gap:10}},React.createElement("div",{style:{minWidth:0}},React.createElement("strong",{style:{fontSize:12.5}},entry.txtName,entry.club?` | ${entry.club}`:""),React.createElement("div",{style:{fontSize:10.5,color:"var(--muted)"}},matched?`${matched.position} · OVR ${matched.overall} · ${matched.club||"Sem clube"}`:"Aguardando resolução")),React.createElement("span",{style:{fontSize:10.5,fontWeight:800,color:entry.status==="matched"||replacement?"var(--green)":entry.status==="ambiguous"||entry.status==="club_mismatch"?"#ffbb26":"var(--danger)"}},replacement?"Substituído":entry.status==="matched"?"Encontrado":entry.status==="owned"?"Já pertence a outro time":entry.status==="ambiguous"?"Ambíguo":entry.status==="club_mismatch"?"Time não confere":entry.status==="duplicate"?"Duplicado":"Não encontrado")),entry.status==="owned"&&React.createElement("button",{type:"button",onClick:()=>replaceLateJoinTxtEntry(index),style:{padding:"7px 9px",borderRadius:9,border:"1px solid var(--border)",background:"var(--surface-soft)",color:"var(--heading)",fontSize:11,fontWeight:800,cursor:"pointer",justifySelf:"start"}},"Substituir por equivalente"),(entry.status!=="matched"&&entry.status!=="replacement"&&entry.status!=="owned")&&React.createElement("select",{value:entry.playerId||"",onChange:(event)=>resolveLateJoinTxtEntry(index,event.target.value),style:{...q,padding:"7px 9px",fontSize:11}},React.createElement("option",{value:""},"Selecionar jogador manualmente"),(entry.candidates&&entry.candidates.length?entry.candidates.map((candidate)=>candidate.player):(catalog||[]).filter((player)=>importNameVariants(player.name).some((name)=>name.includes(normalizeImportText(entry.txtName).split(" ")[0]))).slice(0,30)).map((player)=>React.createElement("option",{key:player.id,value:player.id},player.name," · ",player.position," · OVR ",player.overall," · ",player.club))),replacement&&entry.replacementOf&&matched&&React.createElement("div",{style:{padding:"7px 9px",borderRadius:9,background:"color-mix(in srgb,var(--green) 8%,var(--surface))",border:"1px solid color-mix(in srgb,var(--green) 24%,var(--border))",fontSize:10.5,lineHeight:1.4}},React.createElement("strong",null,entry.replacementOf.name),` (${entry.replacementOf.position} · OVR ${entry.replacementOf.overall}) → `,React.createElement("strong",null,matched.name),` (${matched.position} · OVR ${matched.overall})`),matched&&entry.status==="matched"&&React.createElement("div",{style:{fontSize:10.5,color:"var(--muted)"}},"Vinculado a ",matched.name," · ",matched.club));})),
                    React.createElement("div",{style:{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}},React.createElement("button",{disabled:lateJoinBusy,onClick:()=>setLateJoinOpen(false),style:{...M,marginTop:0,background:"var(--surface-soft)",color:"var(--heading)",border:"1px solid var(--border)"}},"Cancelar"),React.createElement("button",{disabled:lateJoinBusy||!(lateJoinTxtPreview.usableCount||0)||lateJoinTxtPreview.entries.some((entry)=>entry.status!=="matched"&&entry.status!=="replacement"&&entry.status!=="owned"),onClick:confirmLateJoinTxt,style:{...M,...W,marginTop:0,opacity:lateJoinBusy||!(lateJoinTxtPreview.usableCount||0)||lateJoinTxtPreview.entries.some((entry)=>entry.status!=="matched"&&entry.status!=="replacement"&&entry.status!=="owned")?.45:1}},lateJoinBusy?"Adicionando…":"Confirmar entrada"))
                  )
                )
              )
            ),
            ...
            adminSection === "rules" && currentTournament && React.createElement("section", { style:{ ...E,padding:20,marginTop:18 } },
              ...
              currentTournament.type !== "cup" && React.createElement(PlayerOverrideHistoryAdmin, { catalog }),
        ...
        function ChampionshipSummaryModal({ tournament, profiles, onClose }) {
          ...
          if (tournament && tournament.type === "cup") {
            ...
            return ReactDOM.createPortal(React.createElement(ee, { title:tournament.name, onClose },
          ...
          return ReactDOM.createPortal(React.createElement(ee, { title:tournament ? tournament.name : "Resumo do campeonato", onClose },
            ...
            ) : React.createElement("div", { style:{ color:"var(--muted)",textAlign:"center",padding:24 } }, "O snapshot final deste campeonato ainda não está disponível.")), document.body);
        }

        function HeadToHeadStats({ tournament, currentTeam, opponentTeam, profiles, teams, tournaments = [], onOpenChampionshipSummary }) {
          ...
          function biggestResult(team, kind) { let best=null; matches.forEach((match)=>{let r=resultFor(match,team), valid=kind==="win"?r.gf>r.ga:r.gf<r.ga;if(!valid)return;let diff=Math.abs(r.gf-r.ga);if(!best||diff>best.diff||(diff===best.diff&&(kind==="win"?r.gf:r.ga)>best.tie))best={diff,gf:r.gf,ga:r.ga,tie:kind==="win"?r.gf:r.ga};});return best; }
          function scoreHighlight(result, kind) { if(!result)return React.createElement("strong",{style:{fontSize:17}},"—"); let gfColor=kind==="win"?"var(--green)":"var(--heading)",gaColor=kind==="loss"?"var(--danger)":"var(--heading)"; return React.createElement("strong",{style:{fontSize:17,fontVariantNumeric:"tabular-nums"}},React.createElement("span",{style:{color:gfColor}},result.gf),React.createElement("span",{style:{color:"var(--muted)",padding:"0 5px"}},"×"),React.createElement("span",{style:{color:gaColor}},result.ga)); }
          ...
          function trophiesForProfile(profileId) { return (Array.isArray(tournaments)?tournaments:[]).filter((item)=>item&&item.status==="finished").map((item)=>{let standings=Array.isArray(item.finalStandings)?item.finalStandings:[];let entry=standings.find((row)=>row&&String(row.profileId)===String(profileId)&&Number(row.position)===1);return entry?{tournament:item,entry}:null;}).filter(Boolean).sort((a,b)=>(Number(b.tournament.finishedAt)||0)-(Number(a.tournament.finishedAt)||0)); }
          function marketStats(team) { let transfers=Array.isArray(tournament&&tournament.context&&tournament.context.transfers)?tournament.context.transfers:[];let relevant=transfers.filter((item)=>item&&!item.rolledBackAt&&(String(item.fromTeamId||"")===String(team.id)||String(item.toTeamId||"")===String(team.id))&&["market_purchase","market_sale","user_transfer","release_clause"].includes(String(item.type||"")));let spent=relevant.filter((item)=>String(item.toTeamId||"")===String(team.id)).reduce((sum,item)=>sum+Math.abs(Number(item.price)||0),0);return {transactions:relevant.length,spent}; }
          function topScorer() { let scorers=new Map();matches.forEach((match)=>{let goals=Array.isArray(match.goals)?match.goals:Array.isArray(match.scorers)?match.scorers:[];goals.forEach((goal)=>{if(!goal||goal.type==="own_goal")return;let name=goal.playerNameSnapshot||goal.playerName||goal.name;if(!name)return;let key=String(goal.playerId||name);let current=scorers.get(key)||{name,count:0};current.count++;scorers.set(key,current);});});let list=[...scorers.values()].sort((a,b)=>b.count-a.count||String(a.name).localeCompare(String(b.name)));return list[0]||null; }
          ...
          let mineMarket=marketStats(currentTeam),otherMarket=marketStats(opponentTeam),mineTrophies=trophiesForProfile(currentTeam.profileId),otherTrophies=trophiesForProfile(opponentTeam.profileId),carrasco=topScorer();
          function compareRow(label,left,right,format=(value)=>String(value)) { let l=Number(left)||0,r=Number(right)||0,total=l+r,leftPct=total?Math.max(0,Math.min(100,(l/total)*100)):50; let valueNode=(value,side,winning)=>React.createElement("span",{style:{display:"inline-grid",placeItems:"center",minWidth:30,height:30,padding:"0 8px",borderRadius:999,background:winning?(side==="left"?"var(--green)":"color-mix(in srgb, var(--heading) 28%, var(--surface-soft))"):"transparent",color:winning?(side==="left"?"#111513":"#ffffff"):"var(--heading)",WebkitTextFillColor:winning?(side==="left"?"#111513":"#ffffff"):"currentColor",textShadow:"none",fontSize:14,fontWeight:850,fontVariantNumeric:"tabular-nums"}},format(value)); return React.createElement("div",{style:{padding:"12px 0"}},React.createElement("div",{style:{display:"grid",gridTemplateColumns:"84px minmax(0,1fr) 84px",alignItems:"center",gap:10}},React.createElement("div",{style:{textAlign:"left"}},valueNode(left,"left",l>r)),React.createElement("div",{style:{fontSize:12,color:"var(--muted)",textAlign:"center",fontWeight:700}},label),React.createElement("div",{style:{textAlign:"right"}},valueNode(right,"right",r>l))),React.createElement("div",{style:{height:5,borderRadius:99,overflow:"hidden",display:"flex",marginTop:8,background:"var(--surface-soft)"}},React.createElement("div",{style:{width:`${leftPct}%`,background:"var(--green)",transition:"width .25s ease"}}),React.createElement("div",{style:{width:`${100-leftPct}%`,background:"color-mix(in srgb, var(--heading) 28%, var(--surface-soft))",transition:"width .25s ease"}}))); }
          function trophySide(label, profile, trophies, align) { return React.createElement("div",{style:{minWidth:0,textAlign:align}},React.createElement("div",{style:{fontSize:11,color:"var(--muted)",marginBottom:10}},label),trophies.length?React.createElement("div",{style:{display:"flex",gap:9,justifyContent:align==="right"?"flex-end":"flex-start",flexWrap:"wrap"}},trophies.slice(0,6).map(({tournament:item})=>React.createElement("button",{key:item.id,title:item.name,onClick:(event)=>{event.stopPropagation();onOpenChampionshipSummary&&onOpenChampionshipSummary(item)},style:{width:60,height:60,borderRadius:14,border:"1px solid var(--border)",background:"var(--surface-soft)",display:"grid",placeItems:"center",padding:5,cursor:onOpenChampionshipSummary?"pointer":"default"}},React.createElement(TrophyAsset,{tournament:item,size:46})))):React.createElement("div",{style:{fontSize:12,color:"var(--muted)",lineHeight:1.4}},"Nenhum troféu conquistado")); }
          if (!matches.length) return React.createElement("div",null,
            React.createElement("section",{className:"family-card",style:{padding:"34px 20px",textAlign:"center",marginBottom:16}},React.createElement("div",{style:{fontSize:15,fontWeight:850,marginBottom:7}},"Ainda sem confrontos"),React.createElement("div",{style:{fontSize:13,color:"var(--muted)",lineHeight:1.5}},"Vocês ainda não se enfrentaram neste campeonato.")),
            React.createElement("section",{className:"family-card",style:{padding:"18px 20px"}},React.createElement("div",{style:{fontSize:16,fontWeight:850,marginBottom:14}},"Troféus conquistados"),React.createElement("div",{style:{display:"grid",gridTemplateColumns:"minmax(0,1fr) minmax(0,1fr)",gap:18}},trophySide(currentProfile.name||currentTeam.name,currentProfile,mineTrophies,"left"),trophySide(opponentProfile.name||opponentTeam.name,opponentProfile,otherTrophies,"right"))));
          ...
          let totalMatches=matches.length,winPct=totalMatches?(mine.wins/totalMatches)*100:0,drawPct=totalMatches?(mine.draws/totalMatches)*100:0,lossPct=totalMatches?(other.wins/totalMatches)*100:0;
          function resultSegment(value,pct,label,kind){let bg=kind==="win"?"color-mix(in srgb,var(--green) 18%,var(--surface-soft))":kind==="loss"?"color-mix(in srgb,var(--danger) 14%,var(--surface-soft))":"var(--surface-soft)";let color=kind==="win"?"var(--green)":kind==="loss"?"var(--danger)":"var(--heading)";return React.createElement("div",{style:{flex:`${Math.max(pct,18)} 1 0`,minWidth:0,padding:"14px 10px",background:bg,textAlign:"center",borderRadius:12}},React.createElement("div",{style:{fontSize:20,fontWeight:900,fontVariantNumeric:"tabular-nums",lineHeight:1,color}},value),React.createElement("div",{style:{fontSize:10.5,fontWeight:800,marginTop:7,color}},label),React.createElement("div",{style:{fontSize:10.5,color:"var(--muted)",marginTop:3}},`${Math.round(pct)}%`));}
          return React.createElement("div",null,
            React.createElement("section",{className:"family-card",style:{padding:"clamp(18px,4vw,26px)",marginBottom:16}},
              React.createElement("div",{style:{display:"grid",gridTemplateColumns:"repeat(3,minmax(0,1fr))",alignItems:"center",gap:10,marginBottom:18}},
                React.createElement("div",{style:{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:8,minWidth:0,textAlign:"center"}},avatarForTeam(currentTeam,58),React.createElement("strong",{style:{fontSize:14,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",width:"100%",textAlign:"center"}},currentProfile.name||currentTeam.name)),
                React.createElement("div",{style:{display:"flex",alignItems:"center",justifyContent:"center",fontSize:11,color:"var(--muted)",fontWeight:750,textAlign:"center",lineHeight:1.35}},`${totalMatches} ${totalMatches===1?"confronto":"confrontos"}`),
                React.createElement("div",{style:{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:8,minWidth:0,textAlign:"center"}},avatarForTeam(opponentTeam,58),React.createElement("strong",{style:{fontSize:14,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",width:"100%",textAlign:"center"}},opponentProfile.name||opponentTeam.name))
              ),
              React.createElement("div",{style:{display:"flex",gap:5,alignItems:"stretch"}},resultSegment(mine.wins,winPct,"Vitórias","win"),resultSegment(mine.draws,drawPct,"Empates","draw"),resultSegment(other.wins,lossPct,"Derrotas","loss"))
            ),
            React.createElement("section",{className:"family-card",style:{padding:"18px 20px",marginBottom:16}},React.createElement("div",{style:{fontSize:12,fontWeight:800,marginBottom:12}},"Seus últimos confrontos"),React.createElement("div",{style:{display:"flex",gap:8,flexWrap:"wrap",alignItems:"center"}},lastFive.map((result,index)=>{let latest=index===lastFive.length-1;let badge=React.createElement("span",{style:{width:30,height:30,borderRadius:9,display:"grid",placeItems:"center",fontWeight:850,fontSize:12,background:result==="V"?"rgba(0,201,120,.18)":result==="D"?"rgba(255,43,58,.18)":"rgba(255,187,38,.18)",color:result==="V"?"#00c978":result==="D"?"#ff2b3a":"#ffbb26"}},result);return latest?React.createElement("span",{key:index,title:`${result==="V"?"Vitória":result==="D"?"Derrota":"Empate"} · Mais recente`,style:{display:"inline-grid",placeItems:"center",padding:1,border:"1px solid var(--heading)",borderRadius:11,background:"transparent"}},badge):React.createElement("span",{key:index,title:result==="V"?"Vitória":result==="D"?"Derrota":"Empate"},badge)}))),
            React.createElement("section",{className:"family-card",style:{padding:"16px 20px",marginBottom:16}},React.createElement("div",{style:{fontSize:16,fontWeight:850,marginBottom:4}},"Comparativo"),compareRow("Vitórias",mine.wins,other.wins),compareRow("Gols",mine.gf,other.gf),compareRow("Gols por jogo",mine.gpg,other.gpg,(value)=>Number(value).toLocaleString("pt-BR",{minimumFractionDigits:2,maximumFractionDigits:2})),compareRow("Aproveitamento",mine.pointsPct,other.pointsPct,(value)=>`${Math.round(Number(value)||0)}%`),compareRow("Maior sequência",mine.longest,other.longest),compareRow("Transações",mineMarket.transactions,otherMarket.transactions),compareRow("Valor gasto",mineMarket.spent,otherMarket.spent,(value)=>L(Number(value)||0))),
            React.createElement("section",{className:"family-card",style:{padding:"18px 20px",marginBottom:16}},React.createElement("div",{style:{fontSize:16,fontWeight:850,marginBottom:14}},"Troféus conquistados"),React.createElement("div",{style:{display:"grid",gridTemplateColumns:"minmax(0,1fr) minmax(0,1fr)",gap:18}},trophySide(currentProfile.name||currentTeam.name,currentProfile,mineTrophies,"left"),trophySide(opponentProfile.name||opponentTeam.name,opponentProfile,otherTrophies,"right"))),
            matches.length>1 && React.createElement("section",{style:{marginBottom:20}},React.createElement("div",{style:{fontSize:16,fontWeight:850,margin:"0 0 10px"}},"Destaques"),React.createElement("div",{style:{display:"grid",gridTemplateColumns:"repeat(2,minmax(0,1fr))",gap:10}},
              React.createElement("div",{className:"family-card",style:{padding:15}},React.createElement("div",{style:{fontSize:11,color:"var(--muted)",marginBottom:6}},"Maior vitória"),scoreHighlight(biggestResult(currentTeam,"win"),"win")),
              React.createElement("div",{className:"family-card",style:{padding:15}},React.createElement("div",{style:{fontSize:11,color:"var(--muted)",marginBottom:6}},"Maior derrota"),scoreHighlight(biggestResult(currentTeam,"loss"),"loss")),
              React.createElement("div",{className:"family-card",style:{padding:15}},React.createElement("div",{style:{fontSize:11,color:"var(--muted)",marginBottom:6}},"Jogo com mais gols"),React.createElement("strong",{style:{fontSize:17}},`${topGame?topGame.total:0} gols`)),
              React.createElement("div",{className:"family-card",style:{padding:15}},React.createElement("div",{style:{fontSize:11,color:"var(--muted)",marginBottom:6}},"Carrasco"),carrasco?React.createElement(React.Fragment,null,React.createElement("strong",{style:{fontSize:15,display:"block",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}},carrasco.name),React.createElement("div",{style:{fontSize:11.5,color:"var(--muted)",marginTop:4}},`${carrasco.count} ${carrasco.count===1?"gol":"gols"}`)):React.createElement("strong",{style:{fontSize:17}},"—"))
            )),
