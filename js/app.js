      (() => {
        ...
        function to() {
          ...
          function grantBalanceLoan(teamId, requestedAmount) {
            ...
            if(totalGranted+amount>settings.maximumTotalAmount){window.alert(`O teto acumulado deste usuário é ${L(settings.maximumTotalAmount)}.`);return;}
      ...
      `,
        ...
        function BalanceLoanAdmin({tournament,teams,onSave,onGrant}){
          ...
          let field=(label,key,suffix="")=>React.createElement("label",{style:{fontSize:11,color:"var(--muted)"}},label,React.createElement("div",{style:{position:"relative",marginTop:6}},React.createElement("input",{type:"number",min:0,value:values[key],onChange:setNumber(key),style:{...q,paddingRight:suffix?38:12}}),suffix&&React.createElement("span",{style:{position:"absolute",right:12,top:"50%",transform:"translateY(-50%)",fontSize:11}},suffix)));
          return React.createElement("div",{style:{marginTop:16,paddingTop:16,borderTop:"1px solid var(--border)"}},React.createElement("div",{style:{display:"flex",justifyContent:"space-between",alignItems:"center",gap:12,marginBottom:12}},React.createElement("div",null,React.createElement("div",{style:{fontSize:16,fontWeight:850}},"Empréstimos de equilíbrio"),React.createElement("div",{style:{fontSize:11.5,color:"var(--muted)",marginTop:3}},"Compense parte das oportunidades de recompensa perdidas.")),React.createElement("label",{style:{display:"flex",alignItems:"center",gap:8,fontSize:12,fontWeight:800}},React.createElement("input",{type:"checkbox",checked:values.enabled,onChange:event=>setValues({...values,enabled:event.target.checked})}),"Ativo")),React.createElement("div",{style:{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(145px,1fr))",gap:10}},field("Tolerância de jogos","toleranceGames"),field("Diferença mínima","minimumGapPercentage","%"),field("Compensação","compensationPercentage","%"),field("Valor mínimo","minimumAmount","M"),field("Valor máximo inicial","maximumAmount","M"),field("Desconto das recompensas","repaymentPercentage","%")),React.createElement("label",{style:{display:"flex",alignItems:"center",gap:8,fontSize:12,fontWeight:800,margin:"14px 0 10px"}},React.createElement("input",{type:"checkbox",checked:values.allowTopUps,onChange:event=>setValues({...values,allowTopUps:event.target.checked})}),"Permitir complementos"),values.allowTopUps&&React.createElement("div",{style:{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(145px,1fr))",gap:10}},field("Diferença adicional","minimumAdditionalGap"),field("Intervalo entre complementos","minimumDaysBetweenTopUps","dias"),field("Complemento mínimo","minimumTopUpAmount","M"),field("Complemento máximo","maximumTopUpAmount","M"),field("Teto acumulado","maximumTotalAmount","M")),React.createElement("button",{onClick:()=>onSave(values),style:{...M,marginTop:14}},"Salvar configurações"),React.createElement("div",{style:{marginTop:20,fontSize:13,fontWeight:850}},"Análise dos participantes"),React.createElement("div",{style:{display:"grid",gap:10,marginTop:10}},analyses.map(({team,analysis})=>{let existing=analysis.loan,suggestion=existing?analysis.topUpSuggested:analysis.suggested,value=amounts[team.id]!=null?amounts[team.id]:suggestion;return React.createElement("div",{key:team.id,style:{padding:14,borderRadius:15,border:"1px solid var(--border)",background:"var(--surface-soft)"}},React.createElement("div",{style:{display:"flex",justifyContent:"space-between",gap:12}},React.createElement("div",null,React.createElement("strong",null,team.name),React.createElement("div",{style:{fontSize:11.5,color:"var(--muted)",marginTop:4}},`${analysis.played} jogos · referência ${analysis.referenceGames} · média ${L(analysis.averageReward)}`)),React.createElement("strong",{style:{color:suggestion>0?"var(--green)":"var(--muted)"}},suggestion>0?`${existing?"+ ":""}${L(suggestion)}`:"Sem sugestão")),existing&&React.createElement("div",{style:{fontSize:11.5,color:"var(--muted)",marginTop:8}},`Já liberado: ${L(existing.totalGranted)} · falta pagar: ${L(existing.remaining)}`),suggestion>0&&React.createElement("div",{style:{display:"grid",gridTemplateColumns:"1fr auto",gap:8,marginTop:10}},React.createElement("input",{type:"number",min:1,max:Math.max(1,analysis.settings.maximumTotalAmount-analysis.totalGranted),value,onChange:event=>setAmounts({...amounts,[team.id]:Math.max(0,Math.round(Number(event.target.value)||0))}),style:q}),React.createElement("button",{onClick:()=>{let amount=Math.round(Number(value)||0);if(window.confirm(`${existing?"Liberar complemento":"Liberar empréstimo"} de ${L(amount)} para ${team.name}?`))onGrant(team.id,amount)},style:{...M,...W,margin:0,padding:"10px 14px"}},existing?"Liberar complemento":"Dar empréstimo")));}))); 
        }
